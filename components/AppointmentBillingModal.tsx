...
const AppointmentBillingModal: React.FC<AppointmentBillingModalProps> = ({
  ...
  >([]);
  const [selectedPackageTotalAllowed, setSelectedPackageTotalAllowed] =
    useState<number | null>(null);
  const [selectedPackageRemaining, setSelectedPackageRemaining] = useState<
    number | null
  >(null);
  ...
  const [unmatchedOffersDueToMinimum, setUnmatchedOffersDueToMinimum] =
    useState<
      Array<{ offer: Offer; minimumAmount: number; currentAmount: number }>
    >([]);

  // Bundle offer tracking state
  const [matchedBundleOffer, setMatchedBundleOffer] = useState<Offer | null>(
    null,
  );
  const [bundleFreeSessions, setBundleFreeSessions] = useState<string[]>([]);
  const [bundleFreeSessionCount, setBundleFreeSessionCount] =
    useState<number>(0);
  // Track how many units of each treatment should be free (for quantity > 1 scenarios)
  const [_bundleFreeSessionMap, setBundleFreeSessionMap] = useState<
    Map<string, number>
  >(new Map());

  // Track bundle offers that are close to qualifying (for "Buy X more" messages)
  const [partialBundleOffers, setPartialBundleOffers] = useState<
    Array<{
      offer: Offer;
  ...
  const [matchedCashbackOffer, setMatchedCashbackOffer] =
  ...
  const appliedCashbackRef = useRef<{ offerId: string; amount: number } | null>(
    null,
  );
  ...
  const [loadingActivePackageUsage, setLoadingActivePackageUsage] =
    useState(false);
  const [unpaidPackagesTotal, setUnpaidPackagesTotal] = useState<number>(0);
  const [unpaidPackagesBeingPaid, setUnpaidPackagesBeingPaid] = useState<
    Array<{
      packageId: string;
      packageSubId: string;
      amount: number;
  ...
  const [justAddedServiceName, setJustAddedServiceName] = useState<
    string | null
  >(null);
  ...
  const isTreatmentEligibleForOffer = (
    offer: Offer,
    treatment: OfferMatchTreatment,
  ): boolean => {
    ...
    if (offer.doctorIds && Array.isArray(offer.doctorIds) && currentDoctorId) {
      if (
        offer.doctorIds.some((id) => String(id) === String(currentDoctorId))
      ) {
        return true;
      }
    }
      ...
      isEligible = offer.serviceIds.some((svc) => {
        if (typeof svc === "string") {
          return (
            ...
            (treatment.serviceId &&
              String(svc) === String(treatment.serviceId)) ||
          ...
          return (
            ...
            (treatment.serviceId &&
              String(svc._id) === String(treatment.serviceId)) ||
            (svc.serviceSlug &&
              String(svc.serviceSlug) === String(treatment.slug)) ||
            (svc.name &&
              String(svc.name).toLowerCase() ===
                String(treatment.name).toLowerCase())
          );
        }
        return false;
      });
    ...
    if (
      !isEligible &&
      offer.serviceNames &&
      Array.isArray(offer.serviceNames)
    ) {
      isEligible = offer.serviceNames.some(
        (name: string) =>
  ...
  const calculateEligibleTotal = (
    offer: Offer,
    treatments: OfferMatchTreatment[],
  ): number => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
          ...
          if (appointment?.patientId) {
            try {
              ...
              const [patRes, billRes] = await Promise.all([
                axios.get(
                  `/api/clinic/patient-registration?id=${appointment.patientId}`,
                  { headers },
                ),
                axios
                  .get(`/api/clinic/billing-history/${appointment.patientId}`, {
                    headers,
                  })
                  .catch(() => ({ data: { success: false } })),
              ]);
              const patientPkgs: any[] = patRes.data?.patient?.packages || [];
              const billings: any[] = (billRes as any).data?.success
                ? (billRes as any).data.billings || []
                : [];
              const masterIds = new Set(
                masterPackages.map((p: any) => String(p._id)),
              );
              ...
              patientPkgs.forEach((assigned: any) => {
                ...
                const nameFromBilling =
                  billings.find(
                    (b: any) =>
                      b.service === "Package" &&
                      b.package &&
                      (String(b.packageId) === pkgIdStr ||
                        (b.notes && String(b.notes).includes(pkgIdStr))),
                  )?.package || null;
                const resolvedName =
                  snap?.name ||
                  assigned.packageName ||
                  nameFromBilling ||
                  (assigned.totalPrice > 0
                    ? `Package (${pkgIdStr.slice(-6)})`
                    : null);
                if (!resolvedName) return;

                // Treatment resolution: snapshot → billing history treatments
                const resolvedTreatments =
                  Array.isArray(snap?.treatments) && snap.treatments.length > 0
                    ? snap.treatments
                    : (() => {
                        const pkgBill = billings.find(
                          (b: any) =>
                            b.service === "Package" &&
                            b.package === resolvedName &&
                            Array.isArray(b.treatments) &&
                            b.treatments.length > 0,
                        );
                        return pkgBill?.treatments || [];
                      })();

                const resolvedTotalSessions =
                  (snap?.totalSessions ?? 0) ||
                  resolvedTreatments.reduce(
                    (s: number, t: any) => s + (parseInt(t.sessions) || 0),
                    0,
                  );
                ...
                snapshotPackages.push({
                  ...
                  validityInMonths:
                    snap?.validityInMonths ?? assigned.validityInMonths ?? 0,
              ...
              const merged = [...masterPackages, ...snapshotPackages].filter(
                (p: any) => {
              ...
              console.warn(
                "[BillingModal] Snapshot package reconstruction skipped:",
                snapErr,
              );
        ...
        if (appointment?.patientId) {
          try {
            const userPkgRes = await axios.get(
              `/api/clinic/patient-registration?id=${appointment.patientId}`,
              { headers },
            );
            if (
              userPkgRes.data.success &&
              userPkgRes.data.patient?.userPackages
            ) {
              const approvedUserPackages =
                userPkgRes.data.patient.userPackages.filter(
              ...
              for (const pkg of approvedUserPackages) {
                try {
                  ...
                  if (
                    pkgDetailsRes.data.success &&
                    pkgDetailsRes.data.existingPackages
                  ) {
                  ...
                  console.error(
                    `Error fetching userPackage ${pkg.packageId}:`,
                    e,
                  );
                }
              }
                ...
                const results = await Promise.allSettled(
                  departments.map((dept: any) =>
                    axios.get("/api/clinic/services", {
                      headers,
                      params: {
                        departmentId: dept.clinicDepartmentId || dept._id,
                      },
                    }),
                  ),
                );
        ...
        try {
          ...
          if (offersRes.data.success && Array.isArray(offersRes.data.offers)) {
            ...
            const applicableOnes = offersRes.data.offers.filter(
              (o: any) => o.status === "active" && o.enabled === true,
            );
          ...
          if (offerErr.response?.status === 403) {
            ...
            if (
              ...
              setErrors({
                general:
                  "You don't have permission to view billing discounts. Please contact your administrator to enable this feature.",
              });
            }
          }
        }
  ...
  useEffect(() => {
    ...
    const fetchPatient = async () => {
      try {
        ...
        if (
          res.data?.patient?.packages &&
          Array.isArray(res.data.patient.packages)
        ) {
          // Include both Unpaid and Partial payment status packages, but exclude Full
          const unpaidOrPartialPackages = res.data.patient.packages.filter(
            (pkg: any) =>
          ...
          const unpaidTotal = unpaidOrPartialPackages.reduce(
            (sum: number, pkg: any) => {
          ...
          const unpaidPackageDetails = unpaidOrPartialPackages.map(
            (pkg: any) => {
              const remaining = (pkg.totalPrice || 0) - (pkg.paidAmount || 0);
              return {
  ...
  const billedTreatmentInfos = useMemo<BilledTreatmentInfo[]>(() => {
    ...
    billingHistory.forEach((b: any) => {
      ...
      const billedAt =
        b.invoicedDate || b.createdAt || new Date().toISOString();
  ...
  const isTreatmentBilledWithin24Hours = useCallback(
    (treatmentName: string) => {
      ...
      const matchingBillings = billedTreatmentInfos.filter(
        (bt) => bt.name === norm,
      );
      ...
      for (const billing of matchingBillings) {
        ...
        const hoursDiff =
          (now.getTime() - billedAt.getTime()) / (1000 * 60 * 60);
    ...
    const fetchBillingHistory = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          const filteredBillings = (response.data.billings || []).filter(
            (b: any) =>
              !b.isAdvanceOnly &&
          ...
          const cashbackEarnedBillings = (response.data.billings || []).filter(
            (billing: any) => {
              ...
              if (
                ...
                !billing.cashbackAmount ||
                billing.cashbackAmount <= 0
              ) {
                return false;
              }
          ...
          const totalCashbackEarned = cashbackEarnedBillings.reduce(
            (sum: number, billing: any) => {
          ...
          const availableCashbackAmount = Math.max(
            0,
            totalCashbackEarned - totalCashbackUsed,
          );
          ...
          if (cashbackEarnedBillings.length > 0) {
            const sortedByExpiry = cashbackEarnedBillings.sort(
              (a: any, b: any) => {
                return (
                  new Date(a.cashbackEndDate).getTime() -
                  new Date(b.cashbackEndDate).getTime()
                );
              },
            );
          ...
          if (availableCashbackAmount > 0 && nearestExpiry) {
            setAvailableCashback({
              ...
              daysRemaining: Math.ceil(
                (new Date(nearestExpiry).getTime() - today.getTime()) /
                  (1000 * 60 * 60 * 24),
              ),
            });
  ...
  useEffect(() => {
    ...
    const serviceIdsStr = (appointment.serviceIds || [])
      .map((s) => (typeof s === "string" ? s : s?._id))
      .sort()
      .join(",");
    const servicesStr = (appointment.services || [])
    ...
    // Handle multiple services (serviceNames / serviceIds) as fallback
      ...
      appointment.serviceNames.forEach((name, index) => {
        ...
        const slug =
          typeof serviceIdItem === "string"
            ? serviceIdItem
            : serviceIdItem?._id;
        if (name && slug) {
          const matchingTreatment = treatments.find(
            (t) => t.slug === slug || t.name === name,
          );
          const originalQty = 1;
          initialTreatments.push({
            treatmentName: name,
    ...
    // Fallback to single service (serviceName / serviceId)
      ...
      const matchingTreatment = treatments.find(
        (t) => t.slug === slug || t.name === appointment.serviceName,
      );
      const originalQty = 1;
      initialTreatments.push({
  ...
  useEffect(() => {
    ...
    const currentTreatments: OfferMatchTreatment[] = [
      ...selectedTreatments.map((t) => ({
        ...
        serviceId: t.treatmentServiceId,
    ...
    const assignedPkgForOfferMatch = (patientDetails?.packages || []).find(
      (p: any) => String(p.packageId) === String(selectedPackage?._id),
    );
    const isMainPkgForOfferMatch =
      String(patientDetails?.packageId) === String(selectedPackage?._id);
    const existingPkgUsageForOfferMatch = selectedPackage
      ? activePackageUsage.find(
          (p: any) => p.packageName === selectedPackage.name,
        )
      : null;
    const isNewPkgForOfferMatch =
      selectedPackage &&
      !assignedPkgForOfferMatch &&
      !isMainPkgForOfferMatch &&
      !existingPkgUsageForOfferMatch;

    let baseTotal = paidTreatments.reduce(
      (sum, t) => sum + t.price * t.quantity,
      0,
    );
    if (isNewPkgForOfferMatch) {
      const sessionBasedPkgTotal = packageTreatmentSessions
        .filter((t) => t.isSelected)
        .reduce(
          (sum, t) => sum + (t.sessionPrice || 0) * (t.usedSessions || 0),
          0,
        );
      baseTotal =
        baseTotal - sessionBasedPkgTotal + (selectedPackage.totalPrice || 0);
    }
    ...
    billingDebugLog(
      "[OfferMatching] Attempting to match offers for treatments:",
      currentTreatments,
      "Base Total:",
      baseTotal,
    );
    ...
    const applicableOffers = activeOffers.filter((offer) => {
      ...
      if (
        ...
        if (
          ...
          billingDebugLog(
            `[OfferMatching] Offer "${offer.title}" matches for current doctor.`,
          );
          return true;
        }
      }
          ...
          offer.serviceIds.some((svc) => {
            if (typeof svc === "string") {
              ...
              return (
                ...
                (svc.serviceSlug &&
                  String(svc.serviceSlug) === String(t.slug)) ||
                (svc.name &&
                  String(svc.name).toLowerCase() ===
      ...
      if (
        offer.serviceNames &&
        Array.isArray(offer.serviceNames) &&
        offer.serviceNames.length > 0
      ) {
        const matchesServiceName = paidTreatments.some((t) =>
          offer.serviceNames!.some(
            (name: string) =>
              String(name).toLowerCase() === String(t.name).toLowerCase(),
          ),
        );
    ...
    if (applicableOffers.length > 0) {
        ...
        const getDiscountAmount = (offer: Offer) => {
          ...
          if (offer.discountMode === "percentage") {
            ...
            if (offer.maxBenefitCap > 0)
              amount = Math.min(amount, offer.maxBenefitCap);
          } else {
      ...
      billingDebugLog(
        `[OfferMatching] Matched ${applicableOffers.length} offers.`,
      );
      ...
      setAppliedOfferIds((prev) => {
        const filtered = prev.filter((id) =>
          applicableOffers.some((o) => o._id === id),
        );
        if (
  ...
  useEffect(() => {
    ...
    const allowCombining = matchedOffers.every(
      (o) => o.allowCombiningWithOtherOffers,
    );
    const allowReceptionist = matchedOffers.some(
      (o) => o.allowReceptionistDiscount,
    ); // If ANY allows receptionist, show it
  ...
  useEffect(() => {
    ...
    const hasAutoApply = eligibleOffersForAutoApply.some(
      (o) => o.autoApplyBestOffer,
    );
    if (!hasAutoApply) {
    ...
    const currentTreatments = [
      ...
      ...packageTreatmentSessions
        .filter((t) => t.isSelected)
        .map((t) => ({ price: t.sessionPrice, quantity: t.usedSessions })),
    ];

    // For NEW packages (not yet assigned to patient), use full package totalPrice instead of session-based
    const assignedPkgForOffer = (patientDetails?.packages || []).find(
      (p: any) => String(p.packageId) === String(selectedPackage?._id),
    );
    const isMainPkgForOffer =
      String(patientDetails?.packageId) === String(selectedPackage?._id);
    const existingPkgUsageForOffer = selectedPackage
      ? activePackageUsage.find(
          (p: any) => p.packageName === selectedPackage.name,
        )
      : null;
    const isNewPkgForOffer =
      selectedPackage &&
      !assignedPkgForOffer &&
      !isMainPkgForOffer &&
      !existingPkgUsageForOffer;

    let baseTotal = currentTreatments.reduce(
    ...
    matchedOffers.forEach((offer) => {
      ...
      const paidTreatmentsForOffer = currentTreatmentsForOffer.filter(
        (t) => t.price > 0,
      );
      const eligibleTotal = calculateEligibleTotal(
        offer,
        paidTreatmentsForOffer,
      );
    ...
    if (
      showAgentDiscount &&
      agentDiscount &&
      agentDiscount.discountAmount > 0
    ) {
      let agentAmount = 0;
    ...
    if (
      ...
      if (doctorComplaintDiscount.discountType === "percentage") {
        doctorAmount =
          (baseTotal * doctorComplaintDiscount.discountAmount) / 100;
      } else {
    ...
    const bestDiscount = eligibleDiscounts.reduce(
  ...
  useEffect(() => {
    ...
    if (allTreatmentsAreFree) {
      console.log(
        "[BundleMatching] All treatments are free sessions. Skipping bundle matching.",
      );
      clearBundleStateOnce();
      return;
    }
    ...
    // For NEW packages (not yet assigned to patient), use full package totalPrice instead of session-based
    const assignedPkgForBundle = (patientDetails?.packages || []).find(
      (p: any) => String(p.packageId) === String(selectedPackage?._id),
    );
    const isMainPkgForBundle =
      String(patientDetails?.packageId) === String(selectedPackage?._id);
    const existingPkgUsageForBundle = selectedPackage
      ? activePackageUsage.find(
          (p: any) => p.packageName === selectedPackage.name,
        )
      : null;
    const isNewPkgForBundle =
      selectedPackage &&
      !assignedPkgForBundle &&
      !isMainPkgForBundle &&
      !existingPkgUsageForBundle;
    ...
    billingDebugLog(
      "[BundleMatching] Checking for bundle offers. Selected treatments:",
      currentTreatments,
      "Paid treatments:",
      paidTreatments,
    );
    ...
    // Find bundle offers
    const bundleOffers = activeOffers.filter(
      (offer) => offer.offerType === "bundle",
    );
    ...
    if (eligibleBundleOffers.length === 0) {
      billingDebugLog(
        "[BundleMatching] No eligible bundle offers (all created after package).",
      );
      clearBundleStateOnce();
      return;
    }
    ...
    for (const offer of eligibleBundleOffers) {
      ...
      if (
        offer.doctorIds &&
        Array.isArray(offer.doctorIds) &&
        offer.doctorIds.length > 0 &&
        appointment?.doctorId
      ) {
        const appointmentDoctorId = String(appointment.doctorId);
        const isDoctorMatched = offer.doctorIds.some(
          (docId) => String(docId) === appointmentDoctorId,
        );
        ...
        if (isDoctorMatched) {
          ...
          eligibleTreatments.push(
            ...paidTreatments.flatMap((t) => Array(t.quantity).fill(t)),
          );
        }
      }
      ...
      if (
        ...
        offer.serviceIds &&
        Array.isArray(offer.serviceIds) &&
    ...
    if (bestBundleOffer) {
      ...
      setMatchedBundleOffer((prev) => {
        console.log(
          `[BundleMatching] setMatchedBundleOffer callback - prev:`,
          prev?._id,
          "new:",
          bestBundleOffer._id,
          "same?",
          prev?._id === bestBundleOffer._id,
        );
      ...
      setBundleFreeSessions((prev) => {
        console.log(
          `[BundleMatching] setBundleFreeSessions callback - prev:`,
          prev,
          "new:",
          bestFreeSessions,
        );
        if (
          prev.length === bestFreeSessions.length &&
          prev.every((s, i) => s === bestFreeSessions[i])
        ) {
          return prev;
        }
      ...
      setBundleFreeSessionCount((prev) => {
        console.log(
          `[BundleMatching] setBundleFreeSessionCount callback - prev:`,
          prev,
          "new:",
          bestFreeCount,
        );
  ...
  useEffect(() => {
    ...
    // For NEW packages (not yet assigned to patient), use full package totalPrice instead of session-based
    const assignedPkgForCashback = (patientDetails?.packages || []).find(
      (p: any) => String(p.packageId) === String(selectedPackage?._id),
    );
    const isMainPkgForCashback =
      String(patientDetails?.packageId) === String(selectedPackage?._id);
    const existingPkgUsageForCashback = selectedPackage
      ? activePackageUsage.find(
          (p: any) => p.packageName === selectedPackage.name,
        )
      : null;
    const isNewPkgForCashback =
      selectedPackage &&
      !assignedPkgForCashback &&
      !isMainPkgForCashback &&
      !existingPkgUsageForCashback;
    ...
    billingDebugLog(
      "[CashbackMatching] Checking for cashback offers. Selected treatments:",
      currentTreatments,
    );
    billingDebugLog("[CashbackMatching] Base total:", baseTotal);
    billingDebugLog(
      "[CashbackMatching] Total active offers:",
      activeOffers.length,
    );

    // Find cashback offers
    const cashbackOffers = activeOffers.filter(
      (offer) => offer.offerType === "cashback",
    );
    ...
    if (eligibleCashbackOffers.length === 0) {
      billingDebugLog(
        "[CashbackMatching] No eligible cashback offers (all created after package).",
      );
      setMatchedCashbackOffer(null);
      setAppliedCashbackAmount(0);
      return;
    ...
    billingDebugLog(
      "[CashbackMatching] All active offers with types:",
      activeOffers.map((o) => ({
        title: o.title,
    ...
    billingDebugLog(
      "[CashbackMatching] Cashback offers found:",
      ...
      if (
        ...
        for (const treatment of currentTreatments) {
          const isEligible = offer.serviceIds.some((svc) => {
            if (typeof svc === "string") {
              return (
                svc.toLowerCase() === treatment.slug.toLowerCase() ||
                svc.toLowerCase() === treatment.name.toLowerCase()
              );
            }
            return (
              svc.serviceSlug?.toLowerCase() === treatment.slug.toLowerCase() ||
              svc.name?.toLowerCase() === treatment.name.toLowerCase()
            );
          });
    ...
    if (bestCashbackOffer) {
      ...
      const isSameOffer =
        ...
        appliedCashbackRef.current.offerId === bestCashbackOffer._id;
    ...
    const fetchDoctorAppliedDiscountStatus = async () => {
      ...
      try {
        ...
        if (res.data.success && Array.isArray(res.data.complaints)) {
          ...
          const appliedComplaint = res.data.complaints.find((c: any) =>
            Boolean(c?.isDoctorDiscountApplied),
          );
          ...
          if (hasAppliedFromComplaint) {
            setDoctorComplaintDiscount({
              ...
              discountAmount:
                parseFloat(appliedComplaint.doctorDiscountAmount) || 0,
            });
          }
          ...
          const doctorEffectiveDisc = hasAppliedFromComplaint
            ? {
                type: appliedComplaint.doctorDiscountType,
                amount: parseFloat(appliedComplaint.doctorDiscountAmount) || 0,
              }
            : null;

          const agentEffectiveDisc = agentDiscount
            ? {
                type: agentDiscount.discountType,
                amount: agentDiscount.discountAmount,
              }
            : null;
          ...
          const hasOffersWithReceptionistDiscountFalse =
            currentMatchedOffers.length > 0 &&
            currentMatchedOffers.some(
              (o) => o.allowReceptionistDiscount === false,
            );

          // If offers with allowReceptionistDiscount: false are matched, remove agent discount if it was auto-applied
          if (
            hasOffersWithReceptionistDiscountFalse &&
  ...
  useEffect(() => {
    ...
    // Check if any matched offer has allowReceptionistDiscount: false
    const hasOffersWithReceptionistDiscountFalse = matchedOffers.some(
      (o) => o.allowReceptionistDiscount === false,
    );

    if (hasOffersWithReceptionistDiscountFalse && isAgentDiscountApplied) {
      console.log(
        `[OfferWatch] Matched offers have allowReceptionistDiscount: false. Removing auto-applied agent discount.`,
      );
      setIsAgentDiscountApplied(false);
    }
      ...
      try {
        ...
        if (response.data.success && response.data.billings) {
          ...
          allBillings.forEach((billing: any) => {
            if (
              billing.usedFreeSessions &&
              Array.isArray(billing.usedFreeSessions)
            ) {
          ...
          const freeSessions = allBillings
            ...
            .map((billing: any) => {
              ...
              const remainingSessions = offeredSessions.filter(
                (session: string) => {
                  const sessionKey = session.toLowerCase();
                  const usedForThisSession =
                    usedSessionsMap.get(sessionKey) || 0;
              ...
              return {
                ...
                offerName:
                  billing.offerName || billing.offerTitle || "Bundle Offer",
          ...
          console.log(
            "[FreeSessions] Available free sessions (after filtering used):",
            freeSessions,
          );
        } else {
  ...
  useEffect(() => {
    ...
    const treatmentBaseTotal = selectedTreatments.reduce(
    ...
    if (selectedPackage) {
      ...
      const assignedPkg = (patientDetails?.packages || []).find(
        (p: any) => String(p.packageId) === String(selectedPackage._id),
      );
      const mainPackageId = patientDetails?.packageId;
      const isMainPackage =
        String(mainPackageId) === String(selectedPackage._id);
      const existingPkgUsage = activePackageUsage.find(
        (p: any) => p.packageName === selectedPackage.name,
      );
      ...
      if (isNewPackage) {
        ...
        console.log(
          `[NewPackageBilling] New package "${selectedPackage.name}" — using full package price: ${packageBaseTotal}`,
        );
      } else {
          ...
          packageUsageData.billingHistory.forEach((bill: any) => {
            ...
            if (bill.selectedPackageTreatments) {
              bill.selectedPackageTreatments.forEach((bt: any) => {
                const trDef = selectedPackage.treatments.find(
                  (t) => t.treatmentSlug === bt.treatmentSlug,
                );
                if (trDef) {
                  consumedPrepaidAmount +=
                    (trDef.sessionPrice || 0) * (bt.sessions || 0);
                }
              });
            }
          });
        ...
        let availablePrepaidAmount = Math.max(
          0,
          paidAmount - consumedPrepaidAmount,
        );
    ...
    if (
      ...
      if (selectedTreatments.length > 0) {
        const sortedTreatments = [...selectedTreatments].sort(
          (a, b) => a.price - b.price,
        );
        let totalFree = 0;
        let totalDiscount = 0;
        ...
        const updated = sortedTreatments.map((t) => {
          ...
          if (freeAvailable > 0 && t.quantity > 0) {
            ...
            if (qtyFree > 0) {
              ...
              if (remainingQty > 0 && discountPercentage > 0) {
                usesDiscount = true;
                totalDiscount +=
                  (t.price * remainingQty * discountPercentage) / 100;
              }
            }
          ...
          return {
            ...t,
            usesFreeConsultation: usesFree,
            usesMembershipDiscount: usesDiscount,
          };
        });
        ...
        setSelectedTreatments((prev) => {
          if (
            prev.every(
              (t) => !t.usesFreeConsultation && !t.usesMembershipDiscount,
            )
          ) {
            return prev;
          }
          return prev.map((t) => ({
            ...t,
            usesFreeConsultation: false,
            usesMembershipDiscount: false,
          }));
        });
      }
      ...
      if (packageTreatmentSessions.some((t) => t.isSelected)) {
        const hasSessions = packageTreatmentSessions.some(
          (t) => t.isSelected && (t.usedSessions || 0) > 0,
        );
        if (hasSessions) {
          ...
          const withFreeInfo = selectedSessions.map((t: any) => {
            ...
            return {
              ...t,
              sessionsFree,
              remainingSessions: sessions - sessionsFree,
            };
          });
          ...
          withFreeInfo.forEach((t: any) => {
            ...
            sessionUpdates.set(t.treatmentSlug, {
              usesFreeConsultation: usesFree,
              usesMembershipDiscount: usesDiscount,
            });
          });
      ...
      if (
        ...
        packageTreatmentSessions.some((t) => t.isSelected)
      ) {
    ...
    const appliedOffers = matchedOffers.filter((o) =>
      appliedOfferIds.includes(o._id),
    );
    if (appliedOffers.length > 0 && baseTotal > 0) {
      console.log(
        "[OfferDiscountCalc] baseTotal:",
        baseTotal,
        "appliedOffers:",
        appliedOffers.map((o) => ({
          title: o.title,
          discountMode: o.discountMode,
          discountValue: o.discountValue,
          maxBenefitCap: o.maxBenefitCap,
        })),
      );
      appliedOffers.forEach((offer) => {
        if (
          offer.minimumBillAmount === 0 ||
          baseTotal >= offer.minimumBillAmount
        ) {
          ...
          if (
            ...
            matchedBundleOffer &&
            matchedBundleOffer._id === offer._id
          ) {
            ...
            if (offer.discountValue && offer.discountValue > 0) {
              ...
              console.log(
                `[BundleDiscount] Bundle "${offer.title}": Additional discount only = ${additionalDiscount} (free session stored for future)`,
              );
            } else {
              ...
              console.log(
                `[BundleDiscount] Bundle "${offer.title}": No discount in current billing. Free session (${bundleFreeSessions.join(", ")}) stored for future redemption`,
              );
            }
            ...
            if (offer.discountMode === "percentage") {
              currentOfferDiscount =
                (eligibleTotal * offer.discountValue) / 100;
              console.log(
                `[OfferDiscountCalc] Before cap: eligibleTotal=${eligibleTotal}, discountValue=${offer.discountValue}, calculated=${currentOfferDiscount}, maxBenefitCap=${offer.maxBenefitCap}`,
              );
              if (offer.maxBenefitCap > 0) {
                currentOfferDiscount = Math.min(
                  currentOfferDiscount,
                  offer.maxBenefitCap,
                );
                console.log(
                  `[OfferDiscountCalc] After cap applied: ${currentOfferDiscount}`,
                );
              }
            } else {
          ...
          console.log(
            `[OfferDiscountCalc] Offer "${offer.title}" discount: ${currentOfferDiscount}, Total offerDiscountAmount: ${offerDiscountAmount}`,
          );
        }
      });
    }
    ...
    const receptionistDiscountAllowed =
      appliedOffers.length === 0 ||
      appliedOffers.every((o) => o.allowReceptionistDiscount);
    if (receptionistDiscountAllowed) {
      if (
        ...
        if (agentDiscount.discountType === "percentage") {
          receptionistDiscountAmount =
            (baseTotal * agentDiscount.discountAmount) / 100;
        } else {
    ...
    const canCombine =
      appliedOffers.length === 0 ||
      appliedOffers.every((o) => o.allowCombiningWithOtherOffers);
    ...
    if (canCombine) {
      ...
      const highest = discounts.reduce((prev, current) =>
        prev.amount >= current.amount ? prev : current,
      );

      calcMembershipDiscount =
        highest.type === "Membership" ? highest.amount : 0;
      calcOfferDiscount = highest.type === "Offer" ? highest.amount : 0;
      calcReceptionistDiscount =
        highest.type === "Receptionist" ? highest.amount : 0;
  ...
  useEffect(() => {
    ...
    const finalAmountAfterCashback = Math.max(
      0,
      discountedAmount - cashbackDeduction,
    );
    ...
    const appliedPastAdvance54Percent = applyPastAdvance54Percent
      ? Math.min(
          ...
          amountForCredits - appliedAdvance - appliedPastAdvance50Percent,
        )
      : 0;
    ...
    setFormData((prev) => {
      ...
      if (
        ...
        if (
          Math.abs(currentPaid - currentAmount) < 0.01 ||
          prev.paid === "" ||
          prev.paid === "0.00"
        ) {
          if (netDue === 0) {
  ...
  const handleTreatmentToggle = (
    ...
    if (existingIndex >= 0) {
      ...
      const isFreeSessionFromPrevious = availableFreeSessions.some(
        (session: any) =>
          session.offerFreeSession.some(
            (freeTreatment: string) =>
      ...
      const isFreeSessionFromCurrentBundle =
        matchedBundleOffer &&
        appliedOfferIds.includes(matchedBundleOffer._id) &&
        bundleFreeSessions.some(
          (freeTreatment: string) =>
            freeTreatment.toLowerCase() === treatment.name.toLowerCase(),
        );
      ...
      const isFreeSession =
        isFreeSessionFromPrevious || isFreeSessionFromCurrentBundle || false;
      ...
      if (isFreeSession) {
        console.log(
          `[FreeSession] Treatment "${treatment.name}" added as FREE session`,
        );
      }
  ...
  const handlePackageSelect = async (pkg: Package) => {
    ...
    const assignedPkg = (patientDetails?.packages || []).find(
      (p: any) => String(p.packageId) === String(pkg._id),
    );
    ...
    const existingPkgUsage = activePackageUsage.find(
      (p: any) => p.packageName === pkg.name,
    );

    console.log(
      `[FE_DEBUG] Package selected: "${pkg.name}" | isTransferred=${!!pkg.isTransferred} | pkg.totalSessions=${pkg.totalSessions} | pkg.remainingSessions=${pkg.remainingSessions}`,
    );
    console.log(
      `[FE_DEBUG] existingPkgUsage found: ${!!existingPkgUsage} | totalAllowed=${existingPkgUsage?.totalAllowedSessions} | remaining=${existingPkgUsage?.remainingSessions} | transferredSessions=${existingPkgUsage?.transferredSessions}`,
    );
    console.log(
      `[FE_DEBUG] activePackageUsage entries:`,
      activePackageUsage.map((p: any) => ({
        name: p.packageName,
        totalAllowed: p.totalAllowedSessions,
        remaining: p.remainingSessions,
        isTransferred: p.isTransferred,
        transferredSessions: p.transferredSessions,
      })),
    );
    ...
    const pkgWithPaymentInfo = {
      ...pkg,
      paidAmount:
        existingPkgUsage?.paidAmount ||
        assignedPkg?.paidAmount ||
        (isMainPackage ? patientDetails?.packagePaidAmount : 0) ||
        0,
      paymentStatus:
        existingPkgUsage?.paymentStatus ||
        assignedPkg?.paymentStatus ||
        (isMainPackage ? patientDetails?.packagePaymentStatus : "Unpaid") ||
        "Unpaid",
    };
    ...
    if (appointment?.patientId) {
      try {
        ...
        if (response.data.success && response.data.packageUsage.length > 0) {
          ...
          const matchedUsage = response.data.packageUsage.find(
            (p: any) => p.packageName === pkg.name,
          );
          if (matchedUsage) {
            fetchedUsageData = matchedUsage;
            setPackageUsageData(fetchedUsageData);
            setSelectedPackageTotalAllowed(
              fetchedUsageData.totalAllowedSessions ?? pkg.totalSessions,
            );
            setSelectedPackageRemaining(
              fetchedUsageData.remainingSessions ?? (pkg.totalSessions || 0),
            );
            console.log(
              `[FE_DEBUG] API fresh data for "${pkg.name}": totalAllowed=${fetchedUsageData.totalAllowedSessions}, remaining=${fetchedUsageData.remainingSessions}, transferredSessions=${fetchedUsageData.transferredSessions}, totalSessions=${fetchedUsageData.totalSessions}`,
            );
          } else {
            setPackageUsageData(null);
            ...
            console.log(
              `[FE_DEBUG] API response does NOT contain "${pkg.name}" — FALLING BACK to pkg.totalSessions=${pkg.totalSessions}`,
            );
          }
        } else {
          setPackageUsageData(null);
          ...
          setSelectedPackageRemaining(pkg.totalSessions || null);
          console.log(
            `[FE_DEBUG] API returned EMPTY for "${pkg.name}" — FALLING BACK to pkg.totalSessions=${pkg.totalSessions}`,
          );
        }
      } catch (error) {
    ...
    console.log(
      `[FE_DEBUG] Treatment sessions initialized:`,
      sessions.map((t: any) => ({
        name: t.treatmentName,
        maxSessions: t.maxSessions,
        previouslyUsed: t.previouslyUsedSessions,
        remaining: t.maxSessions - t.previouslyUsedSessions,
      })),
    );
    console.log(
      `[FE_DEBUG] selectedPackageTotalAllowed=${selectedPackageTotalAllowed}, selectedPackageRemaining=${selectedPackageRemaining}`,
    );
  ...
  const adjustPackageSessionsToFitRemaining = (
    sessions: any[],
    maxRemaining: number,
  ) => {
    let totalSelected = sessions.reduce(
      (sum, t) => sum + (t.isSelected ? t.usedSessions || 0 : 0),
      0,
    );
    ...
    setPackageTreatmentSessions((prev) => {
      ...
      let tempSessions = prev.map((t) => {
        if (t.treatmentSlug === slug) {
          ...
          const availableForTreatment =
            t.maxSessions - t.previouslyUsedSessions;
      ...
      if (selectedPackageRemaining !== null) {
        tempSessions = adjustPackageSessionsToFitRemaining(
          tempSessions,
  ...
  const generateInvoicePDF = async () => {
    try {
      ...
      doc.text(`Name: ${appointment?.patientName || "-"}`, 14, 50);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(
      `[FE_DEBUG] Submitting package billing: packageName=${selectedPackage?.name}, isTransferred=${!!selectedPackage?.isTransferred}, totalAllowed=${selectedPackageTotalAllowed}, remaining=${selectedPackageRemaining}`,
    );
    console.log(
      `[FE_DEBUG] Package treatments being billed:`,
      packageTreatmentSessions
        .filter((t) => t.isSelected)
        .map((t) => ({
          name: t.treatmentName,
    ...
    try {
      ...
      if (!useMultiplePayments && !formData.paymentMethod)
        fieldErrors.paymentMethod = "Choose payment method";

      // Validate multiple payments if enabled
      if (useMultiplePayments) {
        const validPayments = multiplePayments.filter(
          (p) => p.paymentMethod && parseFloat(p.amount) > 0,
        );
        if (validPayments.length === 0) {
          fieldErrors.paymentMethod =
            "Please add at least one payment method with an amount";
        }
        // Check for any payment with empty payment method
        const hasEmptyPaymentMethod = multiplePayments.some(
          (p) => parseFloat(p.amount) > 0 && !p.paymentMethod,
        );
        if (hasEmptyPaymentMethod) {
          fieldErrors.paymentMethod =
            "Please select a payment method for all payment entries";
        }
      }
      ...
      const hasPackageSessionSelected = packageTreatmentSessions.some(
        (t) => t.isSelected,
      );
      ...
      if (hasPackageSelection && !hasPendingAmount) {
        // Check if this is a NEW package (not yet assigned to the patient)
        const assignedPkgForValidation = (patientDetails?.packages || []).find(
          (p: any) => String(p.packageId) === String(selectedPackage?._id),
        );
        const isMainPkgForValidation =
          String(patientDetails?.packageId) === String(selectedPackage?._id);
        const existingPkgUsageForValidation = selectedPackage
          ? activePackageUsage.find(
              (p: any) => p.packageName === selectedPackage.name,
            )
          : null;
        const isNewPkgForValidation =
          selectedPackage &&
          !assignedPkgForValidation &&
          !isMainPkgForValidation &&
          !existingPkgUsageForValidation;
      ...
      if (
        ...
        if (hasRemainingFreeConsultations) {
          ...
          const treatmentQty = selectedTreatments.reduce(
      ...
      if (
        ...
        if (doctorComplaintDiscount.discountType === "percentage") {
          doctorDiscountApplied =
            (amountAfterMembership * doctorComplaintDiscount.discountAmount) /
            100;
      ...
      if (appliedOffers.length > 0) {
        appliedOffers.forEach((offer) => {
          if (
            offer.minimumBillAmount === 0 ||
            baseAmount >= offer.minimumBillAmount
          ) {
            let currentOfferDiscount = 0;
            const eligibleTotal = calculateEligibleTotal(
              offer,
      ...
      let calcReceptionistDiscount =
        doctorDiscountApplied ||
        (isAgentDiscountApplied
          ? agentDiscount?.discountType === "percentage"
            ? (baseAmount * agentDiscount.discountAmount) / 100
            : agentDiscount?.discountAmount || 0
          : 0);
      ...
      const cashbackAmountToUse =
        useCashback && availableCashback
          ? Math.min(availableCashback.amount, finalAmount)
          : 0;
      ...
      const payload: any = {
        ...
        isFreeConsultation:
          isFreeConsultation && (canCombine || calcMembershipDiscount > 0),
        freeConsultationCount,
        membershipDiscountApplied: calcMembershipDiscount,
        doctorDiscountApplied:
          calcReceptionistDiscount > 0 && isDoctorDiscountApplied
            ? calcReceptionistDiscount
            : 0,
        isDoctorDiscountApplied:
          calcReceptionistDiscount > 0 && isDoctorDiscountApplied,
        doctorDiscountType:
          calcReceptionistDiscount > 0 && isDoctorDiscountApplied
            ? doctorAppliedDiscount
              ? doctorComplaintDiscount?.discountType
              : null
            : null,
        doctorDiscountAmount:
          calcReceptionistDiscount > 0 && isDoctorDiscountApplied
            ? doctorAppliedDiscount
              ? doctorComplaintDiscount?.discountAmount
              : 0
            : 0,
        isAgentDiscountApplied:
          calcReceptionistDiscount > 0 && isAgentDiscountApplied,
        agentDiscountType:
          calcReceptionistDiscount > 0 && isAgentDiscountApplied
            ? agentDiscount?.discountType
            : null,
        agentDiscountAmount:
          calcReceptionistDiscount > 0 && isAgentDiscountApplied
        ...
        offerId:
          calcOfferDiscount > 0 || matchedBundleOffer
            ? appliedOffers[0]?._id
            : null,
        offerTitle:
          calcOfferDiscount > 0 || matchedBundleOffer
        ...
        offerType:
          calcOfferDiscount > 0 || matchedBundleOffer
            ? matchedBundleOffer
              ? matchedBundleOffer.offerType
              : appliedOffers[0]?.offerType
            : null,
        ...
        usedFreeSessions: selectedTreatments
          .filter((t: any) => t.isFreeSession)
          .map((t: any) => t.treatmentName),
        usedFreeSessionCount: selectedTreatments.filter(
          (t: any) => t.isFreeSession,
        ).length,
        // Cashback offer fields
        isCashbackApplied: isCashbackApplied && matchedCashbackOffer !== null,
        cashbackOfferId:
          isCashbackApplied && matchedCashbackOffer
            ? matchedCashbackOffer._id
            : null,
        cashbackOfferName:
          isCashbackApplied && matchedCashbackOffer
            ? matchedCashbackOffer.title
            : null,
        cashbackAmount:
          isCashbackApplied && matchedCashbackOffer ? appliedCashbackAmount : 0,
        discountPercent: (() => {
          if (calcMembershipDiscount > 0) {
            ...
            if (
              isDoctorDiscountApplied &&
              doctorComplaintDiscount?.discountType === "percentage"
            ) {
      ...
      if (selectedPackage) {
        ...
        if (
          selectedPackage?.isUserPackage &&
          selectedPackage?.patientPackageId
        ) {
      ...
      if (unpaidPackagesTotal > 0 && paidAmount >= unpaidPackagesTotal) {
        ...
        console.log(
          "[PackagePayment] Tracking unpaid packages being paid:",
          unpaidPackagesBeingPaid,
        );
      }
      ...
      console.log(
        "[BundleDebug] bundleFreeSessionCount:",
        bundleFreeSessionCount,
      );
      console.log(
        "[BundleDebug] offerFreeSession in payload:",
        payload.offerFreeSession,
      );
      console.log(
        "[BundleDebug] freeOfferSessionCount in payload:",
        payload.freeOfferSessionCount,
      );
      console.log("[FreeSessionDebug] selectedTreatments:", selectedTreatments);
      console.log(
        "[FreeSessionDebug] usedFreeSessions in payload:",
        payload.usedFreeSessions,
      );
      console.log(
        "[FreeSessionDebug] usedFreeSessionCount in payload:",
        payload.usedFreeSessionCount,
      );
      console.log(
        "[FreeSessionDebug] availableFreeSessions:",
        availableFreeSessions,
      );
      console.log(
        "[CashbackDebug] Current state - isCashbackApplied:",
        isCashbackApplied,
      );
      console.log(
        "[CashbackDebug] Current state - appliedCashbackAmount:",
        appliedCashbackAmount,
      );
      console.log(
        "[CashbackDebug] Current state - matchedCashbackOffer:",
        matchedCashbackOffer ? matchedCashbackOffer.title : null,
      );
      console.log(
        "[CashbackDebug] isCashbackApplied in payload:",
        payload.isCashbackApplied,
      );
      console.log(
        "[CashbackDebug] cashbackAmount in payload:",
        payload.cashbackAmount,
      );
      console.log(
        "[CashbackDebug] cashbackOfferName in payload:",
        payload.cashbackOfferName,
      );
      ...
      if (response.data.success) {
        ...
        if (appointment?.patientId) {
          try {
            const headers = getAuthHeaders();
            const [historyResponse, balanceResponse, packageUsageResponse] =
              await Promise.all([
                axios.get(
                  `/api/clinic/billing-history/${appointment.patientId}`,
                  { headers },
            ...
            if (
              ...
              setBalances({
                ...
                claimAmount: balanceResponse.data.balances.claimAmount || 0,
                pendingClaim: balanceResponse.data.balances.pendingClaim || 0,
                pastAdvanceBalance:
            ...
            if (
              packageUsageResponse.data?.success &&
              packageUsageResponse.data?.packageUsage
            ) {
              console.log(
                `[FE_DEBUG] Post-billing refresh - packageUsageResponse:`,
                packageUsageResponse.data.packageUsage.map((p: any) => ({
                  name: p.packageName,
                  totalAllowed: p.totalAllowedSessions,
                  remaining: p.remainingSessions,
                  transferredSessions: p.transferredSessions,
              ...
              console.log(
                `[FE_DEBUG] Post-billing refresh - packageUsageResponse NOT successful or empty`,
              );
            }
            ...
            try {
              ...
              if (
                freeSessionsResponse.data.success &&
                freeSessionsResponse.data.billings
              ) {
                ...
                setAvailableFreeSessions(freeSessions);
                console.log(
                  "[FreeSessions] Refreshed available free sessions after billing:",
                  freeSessions,
                );
              } else {
                setAvailableFreeSessions([]);
  ...
  const allPackagesForDropdown: Package[] = [
    ...packages
      ...
      .map((pkg: any) => {
        ...
        const pkgUsage = activePackageUsage.find(
          (u: any) => u.packageName === pkg.name,
        );
    ...
    ...activePackageUsage
      ...
      .map((u: any) => {
        ...
        return {
          ...
          sessionPrice:
            u.packagePrice && u.totalAllowedSessions
              ? u.packagePrice / u.totalAllowedSessions
              : 0,
  ...
  const filteredAllPackages = allPackagesForDropdown.filter((pkg) => {
    ...
    const hasBillableTreatments = pkg.treatments?.some((t: any) => {
      ...
      const totalAllowed = pkg.isTransferred
        ? pkg.totalSessions
        : pkg.totalSessions || 0;
      if (totalAllowed > 0) {
        ...
        if (pkgUsage) {
          ...
          if (pkgUsage.remainingSessions <= 0 && !pkg.isTransferred)
            return false;

          // Check if this specific treatment has remaining sessions
          const treatmentUsage = pkgUsage.treatments?.find(
            (pt: any) =>
              (pt.treatmentName || pt.name || "").toLowerCase() ===
              tName.toLowerCase(),
          );

          if (treatmentUsage) {
            const maxSessions =
              treatmentUsage.sessions || treatmentUsage.maxSessions || 0;
        ...
        if (packageUsageData && packageUsageData.packages) {
          const legacyPkgUsage = packageUsageData.packages.find(
            (p: any) => String(p.packageId) === String(pkg._id),
          );

          if (legacyPkgUsage) {
            const treatmentUsage = legacyPkgUsage.treatments?.find(
              (pt: any) =>
                (pt.treatmentName || pt.name || "").toLowerCase() ===
                tName.toLowerCase(),
            );
        ...
        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5 truncate">
                      {appointment.emrNumber && `MRN: ${appointment.emrNumber}`}
                    </div>
                  </div>
                </div>
                <button
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 lg:gap-6 text-center">
                <div className="min-w-[80px]">
                  ...
                  <div className="text-[11px] sm:text-xs font-bold text-gray-800 truncate">
                ...
                <div>
                  ...
                  <div className="text-[11px] sm:text-xs font-semibold text-gray-800">
                    {formData.invoicedDate
                      ? new Date(formData.invoicedDate).toLocaleDateString(
                          "en-GB",
                ...
                <div>
                  ...
                  <div className="text-[11px] sm:text-xs font-semibold text-gray-800 truncate max-w-[100px] sm:max-w-none">
                ...
                {(appointment.fromTime || appointment.toTime) && (
                  <div>
                    ...
                    <div className="text-[11px] sm:text-xs font-semibold text-gray-800">
                      ...
                      {appointment.toTime ? `–${appointment.toTime}` : ""}
                    </div>
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={
                    isGeneratingPDF ||
                    (!selectedTreatments.length && !selectedPackage)
                  }
              ...
              <div className="flex items-center gap-3 sm:gap-4 flex-shrink-0">
                <div className="text-center sm:text-right">
                  ...
                  <div
                    className={`text-xs sm:text-sm font-bold ${apiPendingBalance > 0 ? "text-red-600" : "text-gray-400"}`}
                  >
                ...
                <div className="text-center sm:text-right">
                  ...
                  <div
                    className={`text-xs sm:text-sm font-bold ${apiAdvanceBalance > 0 ? "text-emerald-600" : "text-gray-400"}`}
                  >
          ...
          <div className="flex-1 overflow-y-auto min-h-0 bg-gray-50 scrollbar-hide">
            ...
            >
              {errors.general && (
                <div
                  className="bg-red-50 border-l-2 border-red-500 rounded p-2 flex items-start gap-2 text-red-700 shadow-sm animate-in slide-in-from-top-2 fade-in"
                  role="alert"
                >
              ...
              {complaintExists === false && (
                <div
                  className="bg-orange-50 border-l-2 border-orange-500 rounded p-2 flex items-start gap-2 text-orange-700 shadow-sm animate-in slide-in-from-top-2 fade-in"
                  role="alert"
                >
                  <AlertCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 animate-pulse" />
                  <div className="flex-1">
                    ...
                    <p className="text-[10px]">
                      No complaint has been created for this appointment yet.
                      Please create a complaint in the Complaint Module before
                      creating billing.
                    </p>
                  </div>
                </div>
              ...
              {isAlreadyBilled && (
                <div
                  className={`${canRebill ? "bg-blue-50 border-blue-500 text-blue-700" : "bg-amber-50 border-amber-500 text-amber-700"} border-l-2 rounded p-2 flex items-start gap-2 shadow-sm animate-in slide-in-from-top-2 fade-in`}
                  role="alert"
                >
                  <AlertCircle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 animate-pulse" />
                  <div className="flex-1">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <div className="flex gap-4 mb-4">
                      ...
                      <span className="text-[10px] text-gray-500 italic self-center">
                        (You can select treatments and a package together in the
                        same bill)
                      </span>
                    </div>
                    ...
                    {membershipUsage?.hasMembership &&
                      !membershipUsage?.isExpired &&
                      membershipUsage?.hasFreeConsultations &&
                      !isMembershipTransferredOut() && (
                        <div
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-semibold text-gray-900">
                              {membershipUsage.membershipName} – Free
                              Consultations
                            </span>
                            <span
                          ...
                          {membershipUsage.remainingFreeConsultations > 0 && (
                            <p className="mt-1 text-[10px] text-emerald-700">
                              This session will be free. No charge applied.
                            </p>
                          )}
                          {membershipUsage.remainingFreeConsultations === 0 && (
                            <p className="mt-1 text-[10px] text-amber-700">
                              ...
                              {membershipUsage.discountPercentage > 0
                                ? ` ${membershipUsage.discountPercentage}% discount will apply.`
                    ...
                    {membershipUsage?.hasMembership &&
                      membershipUsage?.isExpired && (
                        <div className="rounded-lg border bg-red-50 border-red-200 p-2 mb-3 text-xs text-red-700">
                          ...
                          – Membership expired. Regular pricing applies.
                        </div>
                      )}
                    {membershipUsage?.hasMembership &&
                      !membershipUsage?.isExpired &&
                      ...
                      membershipUsage?.discountPercentage > 0 && (
                        <div className="rounded-lg border bg-blue-50 border-blue-200 p-2 mb-3 text-xs text-blue-700">
                          ...
                          – {membershipUsage.discountPercentage}% discount
                    ...
                    <div className="relative z-20">
                        ...
                        <>
                          ...
                          <div className="relative" ref={treatmentDropdownRef}>
                            ...
                            >
                              ...
                              <ChevronDown
                                className={`w-3 h-3 text-gray-400 transition-transform ${treatmentDropdownOpen ? "rotate-180" : ""}`}
                              />
                            </button>
                            {treatmentDropdownOpen && (
                            ...
                            >
                              <span className="text-gray-500 flex items-center gap-2">
                                ...
                                {selectedPackage?.paymentStatus === "Full" && (
                                  <span className="px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 text-[8px] font-bold uppercase">
                                    Full Paid
                                  </span>
                                )}
                                {selectedPackage?.paymentStatus ===
                                  "Partial" && (
                                  <span className="px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[8px] font-bold uppercase">
                                    Partial Paid (${selectedPackage.paidAmount})
                                  </span>
                                )}
                              </span>
                              <ChevronDown
                                className={`w-3 h-3 text-gray-400 transition-transform ${packageDropdownOpen ? "rotate-180" : ""}`}
                              />
                            </button>
                            {packageDropdownOpen && (
                                  ...
                                  {filteredAllPackages.length === 0 ? (
                                    ...
                                    <div className="p-1">
                                      {filteredAllPackages.map((pkg: any) => {
                                        ...
                                        const remaining =
                                          pkgUsage?.remainingSessions ??
                                          pkg.remainingSessions ??
                                          pkg.totalSessions;
                                        const total =
                                          pkgUsage?.totalAllowedSessions ??
                                          pkg.totalSessions;
                                        // Determine if this package exists in patient's profile
                                        const hasPatientStatus =
                                          pkg.paymentStatus &&
                                          pkg.paymentStatus !== null;
                                        return (
                                          <button
                                            key={pkg._id}
                                            type="button"
                                          ...
                                          >
                                            <div className="flex items-center justify-between">
                                              <div className="font-medium flex items-center gap-1">
                                                ...
                                                {pkg.isDeletedMaster && (
                                                  <span
                                                    className="px-1 py-0.5 rounded bg-orange-50 text-orange-600 text-[7px] font-bold uppercase border border-orange-200"
                                                    title="This package was removed from the clinic catalogue but the patient retains full benefits"
                                                  >
                                                    Catalogue Removed
                                                  </span>
                                                )}
                                              ...
                                              <div className="flex items-center gap-1 flex-wrap justify-end">
                                                {pkg.isTransferred && (
                                                  <span className="px-1 py-0.5 rounded bg-blue-50 text-blue-600 text-[8px] font-bold uppercase border border-blue-100">
                                                    Transferred
                                                  </span>
                                                )}
                                                {hasPatientStatus &&
                                                  pkg.paymentStatus ===
                                                    "Full" && (
                                                    <span className="px-1 py-0.5 rounded bg-green-50 text-green-600 text-[8px] font-bold uppercase border border-green-100">
                                                      Full Paid
                                                    </span>
                                                  )}
                                                {hasPatientStatus &&
                                                  pkg.paymentStatus ===
                                                    "Partial" && (
                                                    <span className="px-1 py-0.5 rounded bg-yellow-50 text-yellow-600 text-[8px] font-bold uppercase border border-yellow-100">
                                                      Partial
                                                    </span>
                                                  )}
                                                {hasPatientStatus &&
                                                  pkg.paymentStatus ===
                                                    "Unpaid" && (
                                                    <span className="px-1 py-0.5 rounded bg-red-50 text-red-500 text-[8px] font-bold uppercase border border-red-100">
                                                      Unpaid
                                                    </span>
                                                  )}
                                              </div>
                      ...
                      {selectedTreatments.length > 0 && (
                        <div className="mt-3 space-y-2">
                          {selectedTreatments.map((treatment) => {
                            const isJustAdded =
                              justAddedServiceName &&
                              treatment.treatmentName === justAddedServiceName;
                            ...
                            const appliedOffersForTreatment =
                              matchedOffers.filter(
                                (offer) =>
                                  appliedOfferIds.includes(offer._id) &&
                                  isTreatmentEligibleForOffer(
                                    offer,
                                    treatmentForOffer,
                                  ),
                              );
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                    ...
                                    <div className="min-w-0">
                                      <div className="text-xs font-semibold text-gray-900 truncate">
                                        ...
                                        {treatment.isFreeSession && (
                                          <span className="ml-1 text-[9px] px-1.5 py-0.5 bg-green-200 text-green-800 rounded-full font-bold">
                                            FREE
                                          </span>
                                        )}
                                      </div>
                                      ...
                                      {(treatment.usesFreeConsultation ||
                                        ...
                                        treatment.isFreeSession ||
                                        appliedOffersForTreatment.length >
                                          0) && (
                                        <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                          ...
                                          {treatment.usesMembershipDiscount && (
                                            <span className="text-[9px] px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded font-semibold">
                                              {membershipUsage?.discountPercentage ||
                                                10}
                                              % Off
                                            </span>
                                          )}
                                          ...
                                          {appliedOffersForTreatment.map(
                                            (offer) => (
                                              <span
                                                key={offer._id}
                                                className="text-[9px] px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded font-semibold flex items-center gap-0.5"
                                              >
                                                🎁 Offer Applied
                                              </span>
                                            ),
                                  ...
                                  <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 flex-wrap">
                                    ...
                                    <button
                                      ...
                                      className="w-6 h-6 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-700 text-xs font-bold"
                                    >
                                      −
                                    </button>
                                    ...
                                    <span className="text-xs font-bold text-gray-900 ml-auto sm:ml-2">
                                      {getCurrencySymbol(currency)}{" "}
                      ...
                      {selectedPackage &&
                        packageTreatmentSessions.length > 0 && (
                          <div className="mt-3 space-y-2">
                            {loadingPackageUsage ? (
                              ...
                              <>
                                ...
                                <div className="p-3 bg-teal-50 border border-teal-200 rounded-lg">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                      {(() => {
                                        const usedFromPrev =
                                          packageTreatmentSessions.reduce(
                                            (sum, t) =>
                                              sum +
                                              (t.previouslyUsedSessions || 0),
                                            0,
                                          );
                                        const selectedNow =
                                          packageTreatmentSessions.reduce(
                                        ...
                                        const totalAllowed =
                                          selectedPackageTotalAllowed || 0;
                                        const usedPrevPercent = Math.min(
                                          (usedFromPrev / totalAllowed) * 100,
                                          100,
                                        );
                                        const selectedPercent = Math.min(
                                          (selectedNow / totalAllowed) * 100,
                                          100 - usedPrevPercent,
                                        );
                                        return (
                                          <>
                                ...
                                {(selectedPackageRemaining ?? 0) <= 0 && (
                                  <div className="mb-2 p-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">
                                    ...
                                    — All {selectedPackageTotalAllowed || 0}{" "}
                                ...
                                <div className="space-y-2">
                                  {(() => {
                                    ...
                                    const paidAmount =
                                      selectedPackage?.paidAmount || 0;
                                    const paymentStatus =
                                      selectedPackage?.paymentStatus ||
                                      "Unpaid";
                                    let consumedPrepaidAmount = 0;
                                    if (packageUsageData?.billingHistory) {
                                      packageUsageData.billingHistory.forEach(
                                        (bill: any) => {
                                          if (bill.selectedPackageTreatments) {
                                            bill.selectedPackageTreatments.forEach(
                                              (bt: any) => {
                                                const trDef =
                                                  selectedPackage.treatments.find(
                                                    (t) =>
                                                      t.treatmentSlug ===
                                                      bt.treatmentSlug,
                                                  );
                                    ...
                                    let availablePrepaid = Math.max(
                                      0,
                                      paidAmount - consumedPrepaidAmount,
                                    );

                                    return packageTreatmentSessions.map(
                                      (treatment) => {
                                        const remainingSessions =
                                          treatment.maxSessions -
                                          treatment.previouslyUsedSessions;
                                        const isFullyUsed =
                                          remainingSessions <= 0;
                                        const isBilledToday =
                                          treatment.isAlreadyBilledForThisAppointment;
                                        ...
                                        if (
                                          ...
                                          treatment.usedSessions > 0
                                        ) {
                                          if (paymentStatus === "Full") {
                                            isPrepaid = true;
                                          } else {
                                            const cost =
                                              treatment.sessionPrice *
                                        ...
                                        return (
                                          ...
                                          >
                                              ...
                                              <div className="flex items-start gap-2 flex-1">
                                                ...
                                                <div>
                                                  ...
                                                  <div className="text-[10px] text-teal-600 font-medium">
                                                    ...
                                                    {treatment.sessionPrice.toFixed(
                                                      2,
                                                    )}
                                                    /session
                                                  </div>
                                                  ...
                                                  0 ? (
                                                    <div
                                                      className={`text-[10px] mt-0.5 font-medium flex items-center gap-1 ${
                                                        isFullyUsed
                                                          ? "text-red-600"
                                                          : "text-amber-600"
                                                      }`}
                                            ...
                                            {errors[
                                              `packageSession_${treatment.treatmentSlug}`
                                            ] && (
                                  ...
                                  <div className="flex gap-4 text-xs">
                                    <span className="text-gray-600">
                                      ...
                                      <span className="font-bold text-teal-600">
                                        {
                                          packageTreatmentSessions.filter(
                                            (t) => t.isSelected,
                                          ).length
                                        }
                                        /{packageTreatmentSessions.length}
                                      </span>
                                    </span>
                                    <span className="text-gray-600">
                                      Sessions:{" "}
                    ...
                    {(() => {
                      ...
                      if (finalMembershipDiscount > 0) {
                        activeDiscounts.push(
                          `Membership (${membershipUsage?.discountPercentage || 0}%)`,
                        );
                      }
                      if (finalOfferDiscount > 0 && appliedOffers.length > 0) {
                        activeDiscounts.push(
                          `Offer: ${appliedOffers.map((o) => o.title).join(", ")}`,
                        );
                      }
                      if (finalReceptionistDiscount > 0) {
                        if (
                          ...
                          activeDiscounts.push(
                            `Dr. Disc (${doctorComplaintDiscount.discountAmount}${doctorComplaintDiscount.discountType === "percentage" ? "%" : " Fixed"})`,
                          );
                        } else if (isAgentDiscountApplied && agentDiscount) {
                          activeDiscounts.push(
                            `Agent Disc (${agentDiscount.discountAmount}${agentDiscount.discountType === "percentage" ? "%" : " Fixed"})`,
                          );
                        }
                      }
                        ...
                        <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                          ...
                          {!canCombine && hasMultipleDiscounts && (
                            <div className="mt-2 text-[9px] text-blue-700 font-bold italic text-center bg-blue-100/50 py-1 rounded">
                              * Other discounts cannot be applied, only the
                              highest one can be applied.
                            </div>
                          )}
                        </div>
                      );
                    ...
                    {isMembershipApplied &&
                      ...
                      totalPrice === 0 && (
                        <div className="mt-3 p-2 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-700">
                          ✓ Free consultation applied — no charge for this
                          session.
                        </div>
                      )}
                  </div>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                        ...
                        !isMembershipTransferredOut() && (
                          ...
                          >
                            <div className="flex items-start gap-2.5">
                              <div
                                className={`p-1.5 rounded-lg ${isMembershipApplied ? "bg-blue-100" : "bg-gray-100"}`}
                              >
                                <Tag
                                  size={14}
                                  className={
                              ...
                              <div className="flex flex-col">
                                <span
                                  className={`text-[10px] font-bold ${isMembershipApplied ? "text-blue-700" : "text-gray-500"}`}
                                >
                                  MEMBERSHIP
                                </span>
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-gray-600 font-medium">
                                    {membershipUsage.remainingFreeConsultations >
                                    0
                                      ? "Free Session"
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (newStatus) {
                                  ...
                                  const anyRestricted = matchedOffers.some(
                                    (o) =>
                                      appliedOfferIds.includes(o._id) &&
                                      !o.allowCombiningWithOtherOffers,
                                  );
                                  if (anyRestricted) {
                                    setAppliedOfferIds([]);
                                  }
                      ...
                      {matchedOffers.map((offer) => {
                        ...
                        const isAutoApplied =
                          offer.autoApplyBestOffer && isApplied;
                        ...
                        const isCashbackAppliedState =
                          isCashbackOffer &&
                          isCashbackApplied &&
                          matchedCashbackOffer?._id === offer._id;
                        const effectiveIsApplied = isCashbackOffer
                          ? isCashbackAppliedState
                          : isApplied;

                        // Check if this is a partial bundle offer
                        const partialBundle = partialBundleOffers.find(
                          (p) => p.offer._id === offer._id,
                        );
                        ...
                        const isOfferCreatedAfterPackage = (() => {
                          ...
                          const packageDate = new Date(
                            selectedPackage.createdAt || 0,
                          );
                          const offerDate = new Date(
                            offer.createdAt || offer.updatedAt || 0,
                          );
                              ...
                              <div className="flex flex-col flex-1 min-w-0">
                                ...
                                {isBundleOffer ? (
                                    ...
                                    {partialBundle && !effectiveIsApplied && (
                                      <div className="mt-1">
                                        <p className="text-[9px] text-blue-700 font-semibold">
                                          Buy {partialBundle.needMore} more{" "}
                                          {partialBundle.needMore === 1
                                            ? "service"
                                            : "services"}{" "}
                                          to apply
                                        </p>
                                        ...
                                        {partialBundle.recommendedTreatments
                                          .length > 0 && (
                                          <div className="flex flex-wrap gap-1 mt-1">
                                            {partialBundle.recommendedTreatments.map(
                                              (treatment, idx) => (
                                                <span
                                                  key={idx}
                                                  className="text-[8px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium"
                                                >
                                                  {treatment}
                                                </span>
                                              ),
                                  ...
                                  <div className="flex flex-col gap-0.5 mt-0.5">
                                    ...
                                    {offer.cashbackExpiryDays &&
                                      offer.cashbackExpiryDays > 0 && (
                                        <span className="text-[8px] text-purple-600 font-medium">
                                          Valid for {offer.cashbackExpiryDays}{" "}
                                          days
                                        </span>
                                      )}
                                  </div>
                                  ...
                                  <div className="flex flex-col gap-0.5 mt-0.5">
                                    ...
                                    {offer.maxBenefitCap > 0 && (
                                      <span className="text-[8px] text-teal-600 font-bold italic">
                                        Up to {getCurrencySymbol(currency)}{" "}
                                        {offer.maxBenefitCap}
                                      </span>
                                    )}
                                  </div>
                                ...
                                {isAutoApplied && (
                                  <span className="text-[8px] text-emerald-600 font-bold mt-0.5">
                                    ✓ Auto-Applied (Best Offer)
                                  </span>
                                )}

                                {isOfferCreatedAfterPackage && (
                                  <span className="text-[8px] text-red-600 font-bold mt-0.5">
                                    ⚠ Offer created after package
                                  </span>
                                )}
                              </div>
                            </div>
                              ...
                              onClick={() => {
                                ...
                                if (isApplied) {
                                  ...
                                  if (offer.offerType === "bundle") {
                                    ...
                                    const paidTreatmentsForBundle =
                                      currentTreatmentsForBundle.filter(
                                        (t) => t.price > 0,
                                      );
                                    ...
                                    for (const treatment of paidTreatmentsForBundle) {
                                      ...
                                      if (
                                        ...
                                        isInBundle = offer.doctorIds.some(
                                          (docId) =>
                                            String(docId) ===
                                            appointmentDoctorId,
                                        );
                                      ...
                                      if (
                                        ...
                                        isInBundle = offer.serviceIds.some(
                                          (svc) => {
                                            if (typeof svc === "string") {
                                              return (
                                                String(svc) ===
                                  ...
                                  if (!offer.allowCombiningWithOtherOffers) {
                                    ...
                                    const hasRestricted = matchedOffers.some(
                                      (o) =>
                                        appliedOfferIds.includes(o._id) &&
                                        !o.allowCombiningWithOtherOffers,
                                    );
                      ...
                      {unmatchedOffersDueToMinimum.length > 0 && (
                        <div className="mt-3 space-y-2">
                          {unmatchedOffersDueToMinimum.map(
                            ({ offer, minimumAmount, currentAmount }) => (
                              <div
                                key={offer._id}
                                ...
                                <div className="flex items-start gap-2">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <p className="text-[9px] text-amber-800 font-semibold mt-1">
                                      Add {getCurrencySymbol(currency)}{" "}
                                      {(minimumAmount - currentAmount).toFixed(
                                        2,
                                      )}{" "}
                      ...
                      {showDoctorDiscount &&
                        ...
                        (appliedOfferIds.length === 0 ||
                          matchedOffers
                            .filter((o) => appliedOfferIds.includes(o._id))
                            .every((o) => o.allowReceptionistDiscount)) && (
                          <div
                            className={`flex items-center justify-between p-2.5 rounded-xl border transition-all ${
                              isDoctorDiscountApplied
                          ...
                          >
                            <div className="flex items-start gap-2.5">
                              <div
                                className={`p-1.5 rounded-lg ${isDoctorDiscountApplied ? "bg-orange-100" : "bg-gray-100"}`}
                              >
                                <TrendingUp
                                  size={14}
                                  className={
                              ...
                              <div className="flex flex-col">
                                <span
                                  className={`text-[10px] font-bold ${isDoctorDiscountApplied ? "text-orange-700" : "text-gray-500"}`}
                                >
                                  DOCTOR DISCOUNT
                                </span>
                                <div className="flex items-center gap-2">
                      ...
                      {showAgentDiscount &&
                        ...
                        !matchedOffers.some(
                          ...
                          >
                            <div className="flex items-start gap-2.5">
                              <div
                                className={`p-1.5 rounded-lg ${isAgentDiscountApplied ? "bg-blue-100" : "bg-gray-100"}`}
                              >
                                <TrendingUp
                                  size={14}
                                  className={
                              ...
                              <div className="flex flex-col">
                                <span
                                  className={`text-[10px] font-bold ${isAgentDiscountApplied ? "text-blue-700" : "text-gray-500"}`}
                                >
                                  AGENT DISCOUNT
                                </span>
                                <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {(isDoctorDiscountApplied ||
                          ...
                          <div className="text-[9px] text-red-500 mt-0.5 font-medium italic">
                            Discounted from {getCurrencySymbol(currency)}{" "}
                            {parseFloat(formData.originalAmount || "0").toFixed(
                              2,
                            )}
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <div className="text-[9px] text-gray-500 mt-0.5">
                          ...
                          {(() => {
                            ...
                            const pendingUsed = balances.pendingBalance || 0;
                            const pendingClaimUsed = balances.pendingClaim || 0;
                            const pendingBeingRolledIn =
                              pendingUsed + pendingClaimUsed;
                            const amountForCredits =
                              finalAmountAfterCashback + pendingBeingRolledIn;
                      ...
                      <div className="mt-3">
                        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-3 space-y-2">
                          ...
                          <label className="flex items-center justify-between cursor-pointer group">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-medium text-gray-700 group-hover:text-gray-900">
                                Use cashback balance now
                              </span>
                            </div>
                            <span className="text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded-full">
                              {getCurrencySymbol(currency)}{" "}
                              {availableCashback.amount.toFixed(2)}
                            </span>
                          </label>
                          ...
                          <div className="text-[9px] text-green-600 mt-1">
                            Expires:{" "}
                            {new Date(
                              availableCashback.expiryDate,
                            ).toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                            })}{" "}
                            ({availableCashback.daysRemaining} days remaining)
                          </div>
                        </div>
                      </div>
                    )}
                          ...
                          <label className="flex items-center justify-between cursor-pointer group">
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setApplyClaimAmount(e.target.checked)
                                }
                                className="w-3.5 h-3.5 text-green-600 rounded focus:ring-green-500 border-gray-300"
                              />
                              <span className="text-[10px] font-medium text-gray-700 group-hover:text-gray-900">
                                Use Insurance Claim Amount
                              </span>
                            </div>
                            <span className="text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded-full">
                              {getCurrencySymbol(currency)}{" "}
                        ...
                        {multiplePayments.map((mp, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            <select
                              ...
                              onChange={(e) => {
                                const u = [...multiplePayments];
                                u[idx] = {
                                  ...u[idx],
                                  paymentMethod: e.target.value,
                                };
                                setMultiplePayments(u);
                              }}
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200 p-3 sm:p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {isMembershipApplied && finalMembershipDiscount > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="font-semibold text-blue-600">
                            −{getCurrencySymbol(currency)}{" "}
                            {finalMembershipDiscount.toFixed(2)}
                          </span>
                        </div>
                      )}
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-[10px] text-blue-700 mt-1 font-semibold">
                                {getCurrencySymbol(currency)}
                                {appliedCashbackAmount.toFixed(2)} will be added
                                to wallet
                              </div>
                              <div className="text-[9px] text-blue-600 mt-1 italic">
                                💡 Cashback expires in{" "}
                                {matchedCashbackOffer.cashbackExpiryDays || 365}{" "}
                                days
                              </div>
                            </div>
                          </div>
                      ...
                      {finalReceptionistDiscount > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-gray-600">
                            {isDoctorDiscountApplied ? "Doctor" : "Agent"}{" "}
                            Discount
                          </span>
                          <span className="font-semibold text-orange-600">
                            −{getCurrencySymbol(currency)}{" "}
                            {finalReceptionistDiscount.toFixed(2)}
                          </span>
                        </div>
                      )}
                      ...
                      {parseFloat(formData.pendingUsed || "0") > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="font-semibold text-amber-600">
                            +{getCurrencySymbol(currency)}{" "}
                            {parseFloat(formData.pendingUsed || "0").toFixed(2)}
                          </span>
                        </div>
                      )}
                      ...
                      {parseFloat(formData.pendingUsed || "0") > 0 &&
                        pendingLedgerRows.length > 0 && (
                          <div className="pl-3 mt-1 mb-1 border-l-2 border-amber-200">
                            ...
                            >
                              {showPendingLedger ? "Hide" : "Show"} pending
                              breakdown ({pendingLedgerRows.length})
                            </button>
                            {showPendingLedger && (
                      ...
                      {parseFloat(formData.advanceUsed || "0") > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="font-semibold text-purple-600">
                            −{getCurrencySymbol(currency)}{" "}
                            {parseFloat(formData.advanceUsed || "0").toFixed(2)}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between text-[11px]">
                        ...
                        <span className="font-bold text-emerald-600">
                          {getCurrencySymbol(currency)}{" "}
                          {parseFloat(formData.paid || "0").toFixed(2)}
                        </span>
                      </div>
                          ...
                          {multiplePayments.map(
                            (payment, idx) =>
                              parseFloat(payment.amount || "0") > 0 && (
                                <div
                                  key={idx}
                                  className="flex items-center justify-between text-[10px] text-gray-500 font-medium"
                                >
                                  <span>
                                    {payment.paymentMethod === "BT"
                                      ? "Bank Transfer (BT)"
                      ...
                      {parseFloat(formData.pending || "0") > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="font-semibold text-amber-600">
                            {getCurrencySymbol(currency)}{" "}
                            {parseFloat(formData.pending || "0").toFixed(2)}
                          </span>
                        </div>
                      )}
                      ...
                      {parseFloat(formData.advance || "0") > 0 && (
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="font-semibold text-purple-600">
                            {getCurrencySymbol(currency)}{" "}
                            {parseFloat(formData.advance || "0").toFixed(2)}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    {loadingMembershipUsage ? (
                      ...
                      <div className="space-y-2">
                        {/* Memberships list */}
                        {patientDetails &&
                        (Array.isArray(patientDetails.memberships)
                          ...
                          return !patientDetails.membershipTransfers?.some(
                            (t: any) =>
                              t.type === "out" &&
                              String(t.membershipId) === String(m.membershipId),
                          );
                        }).length > 0 ? (
                          patientDetails.memberships
                            .filter((m: any) => {
                            ...
                            .map((m: any, idx: number) => {
                              const membershipPlan = memberships.find(
                                (mem: any) => mem._id === m.membershipId,
                              );
                              const displayName =
                                membershipPlan?.name ||
                                m.membershipName ||
                              ...
                              return (
                                <div
                                  ...
                                  className={`rounded-lg border px-3 py-2 ${isExpired ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}
                                >
                                  <div className="flex items-center justify-between">
                                    <div
                                      className={`text-xs font-semibold ${isExpired ? "text-red-800" : "text-emerald-800"}`}
                                    >
                                      {displayName}
                                    </div>
                                    {isExpired ? (
                                      <span className="text-[9px] px-1.5 py-0.5 bg-red-100 text-red-700 rounded-full font-bold uppercase tracking-tighter">
                                        Expired
                                      </span>
                                    ) : (
                                      typeof ml === "number" &&
                                  ...
                                  <div
                                    className={`text-[9px] mt-0.5 ${isExpired ? "text-red-600 font-medium italic" : "text-emerald-600"}`}
                                  >
                                    {m.startDate
                                      ? new Date(
                                          m.startDate,
                        ...
                        {membershipUsage &&
                          ...
                          !isMembershipTransferredOut() && (
                            <div
                              className={`pt-2 mt-1 border-t border-gray-100 space-y-1.5 ${membershipUsage.isExpired ? "opacity-75" : ""}`}
                            >
                              <div className="flex items-center justify-between">
                                <div
                                  className={`text-[10px] font-bold ${membershipUsage.isExpired ? "text-red-700" : "text-gray-700"}`}
                                >
                                  {membershipUsage.membershipName}
                                </div>
                                {membershipUsage.isExpired && (
                                  <span className="text-[8px] px-1.5 py-0.5 bg-red-100 text-red-700 rounded-full font-bold uppercase tracking-tighter shadow-sm">
                                    Expired
                                  </span>
                                )}
                              </div>
                              {!membershipUsage.isExpired ? (
                                <>
                                  <div className="flex items-center justify-between text-[10px]">
                                    ...
                                    <span
                                      className={`font-semibold ${membershipUsage.remainingFreeConsultations > 0 ? "text-emerald-600" : "text-amber-600"}`}
                                    >
                                      {membershipUsage.remainingFreeConsultations ||
                                        0}{" "}
                                      remaining
                                  ...
                                  {membershipUsage.totalFreeConsultations >
                                    0 && (
                                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                      <div
                                        ...
                                        style={{
                                          width: `${Math.min(((membershipUsage.usedFreeConsultations || 0) / membershipUsage.totalFreeConsultations) * 100, 100)}%`,
                                        }}
                                      />
                                    </div>
                                  )}
                                ...
                                <div className="text-[9px] text-red-600 italic font-medium">
                  ...
                  {(loadingFreeSessions ||
                    availableFreeSessions.length > 0) && (
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border-2 border-green-300 p-4 shadow-sm">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-green-700">
                            Earned from bundle offers - Ready to redeem!
                          </div>
                        </div>
                      </div>
                      ...
                      {loadingFreeSessions ? (
                        <div className="flex items-center gap-2 py-2">
                          ...
                          <span className="text-[10px] text-gray-500">
                            Loading free sessions...
                          </span>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          {availableFreeSessions.map(
                            (session: any, index: number) => (
                              <div
                                key={index}
                                className="bg-white rounded-lg p-2.5 border border-green-200"
                              >
                                <div className="flex items-start gap-2">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-[10px] text-green-800 font-semibold mt-0.5">
                                      Free:{" "}
                                      {session.offerFreeSession.join(", ")}
                                    </div>
                                    <div className="text-[9px] text-green-600 mt-0.5">
                                      Earned:{" "}
                                      {session.invoicedDate
                                        ? new Date(
                                            session.invoicedDate,
                                          ).toLocaleDateString("en-US", {
                                            month: "short",
                    ...
                    {loadingActivePackageUsage ? (
                      ...
                      <div className="space-y-2">
                        ...
                        {(() => {
                          ...
                          activePackageUsage.forEach((u) =>
                            usageMap.set(u.packageName, u),
                          );
                          ...
                          const transferredInPackageNames = new Set(
                            activePackageUsage
                              .filter(
                                (u: any) =>
                                  u.isTransferred && u.transferredFrom,
                              )
                              .map((u: any) => u.packageName),
                          );
                          ...
                          const purchasedPackages = (
                            ...
                            const packageName =
                              ...
                              nameFromBillingHist ||
                              (p.totalPrice > 0
                                ? `Package (${String(p.packageId).slice(-6)})`
                                : String(p.packageId));
                            ...
                            if (
                              usageData &&
                              typeof usageData.remainingSessions === "number"
                            ) {
                              ...
                              const hasBeenUsed =
                                (usageData.totalSessions || 0) > 0;
                              if (
                                usageData.totalAllowedSessions === 0 &&
                                !hasBeenUsed
                              ) {
                                // Broken usage data (deleted master with old snapshot): keep it visible
                                return !patientDetails.packageTransfers?.some(
                                  (t: any) =>
                                    t.type === "out" &&
                          ...
                          const transferredInPackages = activePackageUsage
                            ...
                            .map((u: any) => ({
                              ...
                              totalPrice:
                                (u.totalAllowedSessions || 0) *
                                (u.packagePrice || 0), // Estimate
                          ...
                          const allPackages = [
                            ...purchasedPackages.map((p: any) => ({
                              ...
                              isDeletedMaster: !packages.some(
                                (pkg: any) =>
                                  String(pkg._id) === String(p.packageId),
                              ),
                            })),
                            ...approvedUserPackages.map((p: any) => ({
                              ...p,
                              isUserPackage: true,
                            })),
                          ...
                          return allPackages.map(
                            (pkg: any, pkgIndex: number) => {
                              ...
                              if (pkg.isTransferredIn) {
                                ...
                                packageDef = packages.find(
                                  (p) => p.name === packageName,
                                );
                                treatments =
                                  pkg.treatments ||
                                  packageDef?.treatments ||
                                  [];
                                ...
                                packageDef = packages.find(
                                  (p: any) =>
                                    String(p._id) === String(pkg.packageId),
                                );
                                if (packageDef) {
                                  packageName = packageDef.name;
                                  treatments = packageDef.treatments || [];
                                  totalSessions =
                                    packageDef.treatments?.reduce(
                                      (s: number, t: any) =>
                                        s +
                                        (parseInt(t.sessions) ||
                                          t.sessions ||
                                  ...
                                  packageName =
                                    assignedSnap?.name ||
                                    pkg.packageName ||
                                    nameFromBillingHistory ||
                                    (pkg.totalPrice > 0
                                      ? `Package (${pkgIdStr.slice(-6)})`
                                      : pkg.packageId);
                                  treatments =
                                    Array.isArray(assignedSnap?.treatments) &&
                                    assignedSnap.treatments.length > 0
                                      ? assignedSnap.treatments
                                      : [];
                                  totalSessions =
                                    assignedSnap?.totalSessions ||
                                    treatments.reduce(
                                      (s: number, t: any) =>
                                        s + (parseInt(t.sessions) || 0),
                                      0,
                                    );
                                }
                              }
                              ...
                              const totalCashPaidFromBillings =
                                packageBillings.reduce(
                                  (sum: number, b: any) =>
                                    sum + (Number(b.paid) || 0),
                                  0,
                                );
                              const totalAdvanceUsedFromBillings =
                                packageBillings.reduce(
                                  (sum: number, b: any) =>
                                    sum + (Number(b.advanceUsed) || 0),
                                  0,
                                );
                              const totalPaidFromBillings =
                                totalCashPaidFromBillings +
                                totalAdvanceUsedFromBillings;
                              const packagePrice =
                                packageDef?.totalPrice || pkg.totalPrice || 0;
                              ...
                              if (
                                packagePrice > 0 &&
                                totalPaidFromBillings >= packagePrice
                              ) {
                              ...
                              return (
                                <div
                                  key={`${pkg.packageId || pkg._id}-${pkgIndex}`}
                                  className={`border ${isExpired ? "border-red-200 bg-red-50" : "border-teal-200 bg-gradient-to-br from-teal-50 to-cyan-50"} rounded-xl overflow-hidden relative shadow-sm`}
                                >
                                  {isExpired && (
                                  ...
                                  {pkg.isDeletedMaster && !isExpired && (
                                    <div
                                      className="absolute top-0 left-0 px-2 py-0.5 bg-orange-500 text-white text-[7px] font-black uppercase tracking-tighter z-10 rounded-br-lg shadow-sm"
                                      title="This package was removed from the clinic catalogue but the patient retains all purchased benefits."
                                    >
                                      Catalogue Removed
                                    </div>
                                  )}
                                  {pkg.isTransferredIn && (
                                    <div className="absolute top-0 right-0 px-2 py-1 bg-blue-600 text-white text-[8px] font-black uppercase tracking-tighter z-10 rounded-bl-lg shadow-sm">
                                      Transferred In from{" "}
                                      {pkg.transferredFromName || "Patient"}
                                    </div>
                                  )}
                                  <button
                                    type="button"
                                    onClick={() =>
                                      togglePackageExpansion(packageName)
                                    }
                                    className={`w-full px-3 py-2 flex items-center justify-between hover:bg-teal-100/50 transition-colors ${isExpired ? "opacity-75" : ""}`}
                                  >
                                    <div className="flex items-center gap-2">
                                      <div
                                        className={`w-5 h-5 rounded-full ${isExpired ? "bg-red-500" : "bg-teal-500"} flex items-center justify-center`}
                                      >
                                      ...
                                      <div className="text-left">
                                        <div
                                          className={`text-[10px] font-bold ${isExpired ? "text-red-900 line-through" : "text-gray-900"} flex items-center gap-1.5`}
                                        >
                                          {packageName}
                                          {!isExpired &&
                                            paymentStatus === "Full" && (
                                              <span className="px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 text-[7px] font-black uppercase tracking-tighter shadow-sm">
                                                Full Paid
                                              </span>
                                            )}
                                          {!isExpired &&
                                            paymentStatus === "Partial" && (
                                              <span className="px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[7px] font-black uppercase tracking-tighter shadow-sm">
                                                Partial (₹{totalPaid.toFixed(0)}
                                                )
                                              </span>
                                            )}
                                          {!isExpired && isUnpaid && (
                                            <span className="px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 text-[7px] font-black uppercase tracking-tighter shadow-sm">
                                              Unpaid
                                            </span>
                                          )}
                                          {!isExpired && advanceUsed > 0 && (
                                            <span className="px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[7px] font-black uppercase tracking-tighter shadow-sm">
                                              Advance Used
                                            </span>
                                          )}
                                        </div>
                                        <div className="text-[8px] text-gray-500 flex items-center gap-2">
                                          <span>
                                            {displayData.treatments?.length ||
                                              0}{" "}
                                            treatments ·
                                            {usageData?.totalAllowedSessions ??
                                              displayData.totalSessions ??
                                              0}{" "}
                                            total ·
                                            {usageData?.remainingSessions ??
                                              displayData.totalSessions ??
                                              0}{" "}
                                            remaining
                                          </span>
                                          {(pkg.startDate || pkg.endDate) && (
                                            <>
                                              ...
                                              <span
                                                className={`flex items-center gap-0.5 ${isExpired ? "text-red-600 font-bold" : ""}`}
                                              >
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      <span
                                        className={`text-[8px] px-1.5 py-0.5 rounded-full font-medium ${isExpired ? "bg-red-100 text-red-700" : "bg-teal-100 text-teal-700"}`}
                                      >
                                        {displayData.billingHistory?.length ||
                                          0}{" "}
                                        bills
                                      </span>
                                      {isExpanded ? (
                                  ...
                                  {isExpanded && (
                                    <div className="border-t border-teal-200 p-2.5 space-y-2">
                                      ...
                                      displayData.treatments.length > 0 ? (
                                        displayData.treatments.map(
                                          (treatment: any, tIndex: number) => {
                                            const treatmentUsage =
                                              usageData?.treatments?.find(
                                                (t: any) =>
                                                  t.treatmentSlug ===
                                                  treatment.treatmentSlug,
                                              );
                                            const maxSessions =
                                              treatmentUsage?.maxSessions ||
                                              treatment.sessions ||
                                              treatment.maxSessions ||
                                              0;
                                            const totalUsedSessions =
                                              treatmentUsage?.totalUsedSessions ||
                                              0;
                                            const remainingSessions =
                                              maxSessions - totalUsedSessions;
                                            const isFullyUsed =
                                              maxSessions > 0 &&
                      ...
                      <div className="space-y-2">
                              ...
                              <div>
                                ...
                                <div className="text-[10px] text-gray-500">
                                  {billing.paymentMethod ||
                                    (billing.multiplePayments?.length > 0
                                      ? billing.multiplePayments
                                          .map((mp: any) => mp.paymentMethod)
                                          .join(" + ")
                                      : "–")}
                        ...
                        {last3Billings.length > 0 && (
                          <button
                            type="button"
                            onClick={() => {
                              const router = window.location.href.includes(
                                "/clinic/",
                              )
                              ...
                              if (router) {
                                router.push(
                                  `/clinic/billing-history?patientId=${appointment.patientId}`,
                                );
                              } else {
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {dept.services.map((svc) => {
                                  ...
                                  const serviceIdentifier =
                                    svc.serviceSlug || svc._id;
                                  const isAdded = selectedTreatments.some(
                                    (t) =>
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="text-center w-full">
                                        <span className="text-[10px] font-semibold text-gray-800 line-clamp-2 leading-tight min-h-[20px]">
                                          {svc.name}
                                        </span>
                                      </div>
                                      <div className="flex items-center gap-1.5">
                                            ...
                                            onClick={async () => {
                                              ...
                                              if (
                                                ...
                                                try {
                                                  ...
                                                  console.error(
                                                    "Error saving smart recommendation:",
                                                    error,
                                                  );
                                                }
                                              }
