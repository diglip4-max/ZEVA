...
const DashboardMetricCard: React.FC<DashboardMetricCardProps> = ({
  ...
  return (
    ...
    >
      <Card className="h-full transition-colors duration-200 bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium text-gray-600 dark:text-gray-400">
            {title}
          </CardTitle>
          {IconComponent && (
            <IconComponent className="h-4 w-4 text-gray-600 dark:text-gray-400" aria-hidden="true" />
          )}
        </CardHeader>
        <CardContent>
