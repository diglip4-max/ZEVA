...
/* Productivity Bar Component (Same as AgentDesktime) */
...
/* Main WorkSessionModal Component */
const WorkSessionModal = ({
  isOpen,
  onClose,
  userId,
  userName,
  ...
  console.log('Work session model state:', session);
  ...
  const getBaseUrl = () => {
    if (userRole === 'doctor' || userRole === 'doctorStaff') {
      return '/api/doctor/work-session/';
    }
    return '/api/agent/work-session/';
  };

  // Load all data
  const loadData = async () => {
    if (!isOpen || !userId || !clinicToken) return;
    ...
    try {
      ...
      console.log('Loading data for:', { userId, userName, userRole, baseUrl });

      // Load daily session
      const sessionRes = await axios.get(`${baseUrl}today?userId=${userId}`, {
      ...
      const hourlyRes = await axios.get(`${baseUrl}hourly?date=${formattedDate}&userId=${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      console.log('Hourly response:', hourlyRes.data);

      if (hourlyRes.data?.success) {
        setHourlyData(hourlyRes.data.data);
      } else {
        // Set empty hourly data
      ...
      // Load weekly stats
      const weeklyRes = await axios.get(`${baseUrl}weekly?offset=${weekOffset}&userId=${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      // Load monthly stats
      const monthlyRes = await axios.get(`${baseUrl}monthly?offset=${monthOffset}&userId=${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      console.error('Error loading work session data:', error);
      toast.error('Failed to load work session data');
    } finally {
      setLoading(false);
    }
  };

  // Load data when modal opens or view changes
  useEffect(() => {
    if (isOpen) {
      loadData();
    }
  }, [isOpen, viewMode, weekOffset, monthOffset]);
...
// };
  ...
  // Render Daily View (Same as AgentDesktime)
    ...
    return (
      <>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Arrival Time" value={session.arrivalTime || '—'} icon={<LogIn className="text-blue-400" />} color="blue" />
          <StatCard
            title="Logout Time"
          ...
          />
          <StatCard title="Productive Time" value={formatTime(session.productiveTime)} icon={<MousePointer className="text-green-400" />} color="green" />
          <StatCard title="DeskTime" value={formatTime(session.deskTime)} icon={<Monitor className="text-purple-400" />} color="purple" />
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <StatCard title="Time at Work" value={formatTime(timeAtWork)} icon={<Briefcase className="text-yellow-400" />} color="yellow" fullWidth />
  ...
  // Render Weekly View (Same as AgentDesktime)
  const renderWeeklyView = () => (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Total DeskTime" value={formatTime(weeklyStats.totalDeskTime)} icon={<Monitor className="text-purple-400" />} color="purple" />
        <StatCard title="Total Productive" value={formatTime(weeklyStats.totalProductiveTime)} icon={<MousePointer className="text-green-400" />} color="green" />
        <StatCard title="Avg Effectiveness" value={`${weeklyStats.avgProductivity.toFixed(1)}%`} icon={<TrendingUp className="text-blue-400" />} color="blue" />
        <StatCard title="Avg DeskTime/Day" value={formatTime(getWeeklyAverageTime())} icon={<Activity className="text-yellow-400" />} color="yellow" />
      </div>
            ...
            <div key={i} className={`rounded-lg p-4 ${day.hasSession ? 'bg-gray-700' : 'bg-gray-900'}`}>
              ...
              <div className="space-y-2">
                {/* <div className="text-center">
                  ...
                  <div className="font-semibold">{formatTime(day.arrivalTime) || 0}</div>
                </div> */}
                <div className="text-center">
  ...
  // Render Monthly View (Same as AgentDesktime)
  const renderMonthlyView = () => (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Total DeskTime" value={formatTime(monthlyStats.totalDeskTime)} icon={<Monitor className="text-purple-400" />} color="purple" />
        <StatCard title="Total Productive" value={formatTime(monthlyStats.totalProductiveTime)} icon={<MousePointer className="text-green-400" />} color="green" />
        <StatCard title="Avg Effectiveness" value={`${monthlyStats.avgProductivity.toFixed(1)}%`} icon={<TrendingUp className="text-blue-400" />} color="blue" />
        <StatCard title="Working Days" value={`${monthlyStats.totalDays} days`} icon={<Calendar className="text-yellow-400" />} color="yellow" />
      </div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
        <div className="bg-gray-900 rounded-xl w-full max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
          {/* Header */}
          <div className="p-6 border-b border-gray-800 flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-white">
                {userName}'s Work Session
              </h1>
            ...
            {loading ? (
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  ...
                  <p className="mt-4 text-gray-400">Loading work session data...</p>
                </div>
              </div>
            ) : (
              renderViewContent()
