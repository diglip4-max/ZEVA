...
export default function AppointmentBookingModal({
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden flex flex-col transform transition-all duration-200 scale-100 opacity-100"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          <h2 id="modal-title" className="text-base font-semibold text-gray-900">Book Appointment</h2>
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-gray-900"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="rounded-lg border border-gray-200 bg-gradient-to-br from-blue-50/50 to-indigo-50/30 p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                {selectedDoctor?.email && (
                  <p className="text-[10px] text-gray-600 truncate">{selectedDoctor.email}</p>
                )}
              </div>
              <div className="text-[10px] text-gray-700">
                ...
                {doctorDeptLoading ? (
                  ...
                  <p className="text-red-600 text-[10px]">{doctorDeptError}</p>
                ) : departmentNames.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {departmentNames.map((name, idx) => (
                      <span
                        key={`${name}-${idx}`}
                        className="px-1.5 py-0.5 rounded bg-white border border-gray-200 text-[9px] text-gray-700"
                      >
                        {name}
                      </span>
                    ))}
                  ...
                  <p className="text-gray-600 text-[10px]">No departments</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {/* Room Field */}
            <div>
              <label htmlFor="room-select" className="block text-xs font-medium text-gray-700 mb-1.5">
              ...
              <select
                ...
                className={`w-full border rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all ${
              ...
              {fieldErrors.roomId && (
                <p id="room-error" className="mt-1 text-[10px] text-red-600" role="alert">{fieldErrors.roomId}</p>
              )}
            </div>

            {/* Doctor Field */}
            <div>
              <label htmlFor="doctor-select" className="block text-xs font-medium text-gray-700 mb-1.5">
              ...
              {fieldErrors.doctorId && (
                <p id="doctor-error" className="mt-1 text-[10px] text-red-600" role="alert">{fieldErrors.doctorId}</p>
              )}
            </div>

            {/* Status Field */}
            <div>
              <label htmlFor="status-select" className="block text-xs font-medium text-gray-700 mb-1.5">
              ...
              {fieldErrors.status && (
                <p id="status-error" className="mt-1 text-[10px] text-red-600" role="alert">{fieldErrors.status}</p>
              )}
            </div>
          </div>
          ...
          {/* Search Patient */}
          <div>
            <label htmlFor="patient-search" className="block text-xs font-medium text-gray-700 mb-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full border rounded-lg pl-9 pr-9 py-2 text-xs focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all ${
              ...
              />
              {searching && (
                <Loader2 className="absolute right-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 text-gray-400 animate-spin" />
              )}
            </div>
            {fieldErrors.patientId && (
              <p id="patient-error" className="mt-1 text-[10px] text-red-600" role="alert">{fieldErrors.patientId}</p>
            )}
              ...
              <div className="mt-2 border border-gray-200 rounded-lg max-h-48 overflow-y-auto">
                {searchResults.map((patient) => (
                  <div
                    ...
                    className="p-2.5 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                  >
                    <p className="font-medium text-xs text-gray-900">{patient.fullName}</p>
                    <p className="text-[10px] text-gray-500">{patient.mobileNumber} • {patient.email || "No email"}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {selectedPatient && (
              <div className="mt-2 p-2.5 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-xs font-medium text-blue-900">Selected: {selectedPatient.fullName}</p>
              </div>
            )}
            ...
            <button
              ...
              className="mt-2 flex items-center gap-2 text-blue-600 hover:text-blue-700 text-xs font-medium"
            >
          ...
          {showAddPatient && (
            <div className="border border-gray-200 rounded-lg p-4 space-y-4 bg-gray-50">
              <h3 className="text-xs font-medium text-gray-900">Add New Patient</h3>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">EMR Number</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Patient Type</label>
                  <select
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className={`w-full border rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-gray-900 focus:border-gray-900 ${
              ...
              {fieldErrors.followType && (
                <p className="mt-1 text-[10px] text-red-600">{fieldErrors.followType}</p>
              )}
            </div>

            {/* Referral */}
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">Referral</label>
              <select
                value={referral}
                onChange={(e) => setReferral(e.target.value)}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-gray-900 focus:border-gray-900"
              >
            ...
            {/* Emergency */}
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">Emergency</label>
              <select
                value={emergency}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              {fieldErrors.startDate && (
                <p className="mt-1 text-[10px] text-red-600">{fieldErrors.startDate}</p>
              )}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">
              ...
              {fieldErrors.fromTime && (
                <p className="mt-1 text-[10px] text-red-600">{fieldErrors.fromTime}</p>
              )}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">
              ...
              />
              {fieldErrors.toTime && (
                <p className="mt-1 text-[10px] text-red-600">{fieldErrors.toTime}</p>
              )}
            </div>
          </div>
          ...
          {/* Actions */}
          <div className="flex gap-3 pt-4 border-t border-gray-200 sticky bottom-0 bg-white -mx-6 px-6 pb-0">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-xs font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 px-4 py-2 bg-gray-900 text-white rounded-lg text-xs font-medium hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 transition-colors"
            >
