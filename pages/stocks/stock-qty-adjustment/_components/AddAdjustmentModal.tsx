...
import { PlusCircle, X, Plus, Trash2, CirclePlus } from "lucide-react";
...
const AddAdjustmentModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post(
        "/api/stocks/stock-qty-adjustment/add",
        {
          ...formData,
          items,
        },
      ...
      if (res.data?.success) {
        ...
        setError(res.data?.message || "Failed to create adjustment");
      }
    } catch (err: any) {
      setError(
        err.response?.data?.message || "Error creating stock adjustment",
      );
    } finally {
      setLoading(false);
    }
