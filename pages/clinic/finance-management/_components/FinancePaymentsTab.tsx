...
function PaymentDetailsView({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {fields
          ...
          .map((f) => (
            <div
              key={f.label}
              className={`rounded-xl border border-[#EDE7DA] dark:border-[#1f2e29]/60 bg-gradient-to-br ${f.accent} p-3.5 ${f.span === 2 ? "sm:col-span-2" : ""}`}
            >
      ...
      {!payment.reversed && (
        <div className="pt-4 border-t border-[#EDE7DA] dark:border-[#1f2e29]/60 flex items-center justify-between gap-3">
...
function StatsSection({
  ...
  const stats = [
    ...
    { label: "Cheque Payments", value: summary.chequeCount },
    {
...
// ============================================================
// PAYMENT METHOD BREAKDOWN — right-side panel, totals per
// method from the currently loaded payments (this page's data)
// ============================================================
function PaymentMethodBreakdown({
  ...
  const totals = PAYMENT_METHODS.map((m) => {
    const rows = payments.filter((p) => p.method === m.value && !p.reversed);
    const total = rows.reduce((sum, p) => sum + (p.amount || 0), 0);
    return { method: m.value as PaymentMethod, label: m.label, total };
  });
  ...
  return (
    <div className="bg-white dark:bg-[#111d19] rounded-2xl border border-[#EDE7DA] dark:border-[#1a2622] shadow-sm overflow-hidden h-fit">
            ...
            return (
              <div key={t.method}>
                ...
                <div className="relative w-full h-1.5 rounded-full bg-[#F1ECE0] dark:bg-[#16231f] overflow-hidden">
                  <div
                    className={`absolute inset-y-0 left-0 rounded-full ${meta.dot}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
      ...
      <p className="px-5 pb-4 text-[10px] text-stone-400 dark:text-stone-500">
        Based on the payments currently shown below.
      </p>
    </div>
  );
}
...
const FinancePaymentsTab: React.FC<UseFinancePermissionReturn> = ({
  ...
  return (
    <div className="space-y-7">
      ...
      {/* Two-column layout: payment activity (main) + method breakdown (side) */}
      <div className="grid grid-cols-1 gap-6 items-start">
        {/* Right side: Payment Method Breakdown */}
        <PaymentMethodBreakdown payments={payments} currency={currency} />
