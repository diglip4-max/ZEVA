...
function AppointmentPage({
  ...
  // Block / Unblock slot state
  const [blockedSlots, setBlockedSlots] = useState<BlockedSlotEntry[]>([]);
  const [blockedSlotsLoading, setBlockedSlotsLoading] =
    useState<boolean>(false);
  const [blockSlotModal, setBlockSlotModal] = useState<{
    isOpen: boolean;
  ...
  const [unblockConfirm, setUnblockConfirm] = useState<BlockedSlotEntry | null>(
    null,
  );
  ...
  // Load blocked slots for the currently selected date
  const loadBlockedSlots = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `/api/clinic/blocked-slots?date=${selectedDate}`,
        { headers: getAuthHeaders() },
      );
      if (res.data.success) {
        setBlockedSlots(res.data.blockedSlots || []);
      } else {
        // Silent: keep existing state on error
        setBlockedSlots([]);
      }
    } catch (err: any) {
      const status = err?.response?.status;
      if (status === 403) {
        // Permission issue - clear the list quietly
        setBlockedSlots([]);
      } else {
        // Other errors are also silent to avoid noisy toasts
        setBlockedSlots([]);
      }
    } finally {
      setBlockedSlotsLoading(false);
    }
  }, [selectedDate, routeContext, permissions.canRead, getAuthHeaders]);
  ...
  // Helper: check whether a given (doctorId, roomId, sub-slot start, sub-slot end) is currently blocked
  const getBlockingForCell = (
    doctorId: string | null,
    roomId: string | null,
    subStartMinutes: number,
    subEndMinutes: number,
  ): BlockedSlotEntry | null => {
    if (!blockedSlots || blockedSlots.length === 0) return null;
    for (const b of blockedSlots) {
      if (!b.isActive) continue;
      // Block must reference either this doctor or this room
      const doctorMatch = b.doctorId && doctorId && b.doctorId === doctorId;
      const roomMatch = b.roomId && roomId && b.roomId === roomId;
      if (!doctorMatch && !roomMatch) continue;
      const bFrom = timeStringToMinutes(b.fromTime);
      const bTo = timeStringToMinutes(b.toTime);
      // Overlap test
      if (subStartMinutes < bTo && subEndMinutes > bFrom) {
        return b;
      }
    }
    return null;
  ...
  // Get the full blocked range (in minutes) that intersects with the given row,
  // for a specific doctor or room. Used to render the overlay exactly once per row.
  const getRowBlockedSegments = (
    doctorId: string | null,
    roomId: string | null,
    rowStartMinutes: number,
    rowEndMinutes: number,
  ): Array<{
    block: BlockedSlotEntry;
    fromMinutes: number;
    toMinutes: number;
    ...
    for (const b of blockedSlots) {
      ...
      const bFrom = timeStringToMinutes(b.fromTime);
      const bTo = timeStringToMinutes(b.toTime);
      const from = Math.max(bFrom, rowStartMinutes);
      const to = Math.min(bTo, rowEndMinutes);
      if (to > from) {
        segments.push({ block: b, fromMinutes: from, toMinutes: to });
      }
    }
    return segments;
  };
  ...
  const openBlockSlotModal = (opts?: {
    ...
    if (!permissions.canCreate) {
      showErrorToast("You do not have permission to block time slots");
      return;
    }
    setBlockSlotModal({
      isOpen: true,
      defaultFromTime: opts?.fromTime || "",
      defaultToTime: opts?.toTime || "",
      defaultDoctorId: opts?.doctorId || "",
      defaultRoomId: opts?.roomId || "",
    });
  };

  const handleUnblock = async (block: BlockedSlotEntry) => {
    setUnblockLoading(true);
    try {
      const res = await axios.patch(
        `/api/clinic/blocked-slots/${block._id}`,
        { action: "unblock" },
        { headers: getAuthHeaders() },
      );
      if (res.data.success) {
        toast.success("Time slot unblocked", { duration: 2000 });
        setUnblockConfirm(null);
        await loadBlockedSlots();
      } else {
        toast.error(res.data.message || "Failed to unblock slot");
      }
    } catch (err: any) {
      toast.error(
  ...
  */
  ...
  > = {
    ...
    "no-show": { bg: "#c04e4eff", text: "#ffffff", border: "#c04e4eff" },
  ...
  const handleColorChange = (status: string, newColor: string) => {
    ...
    const updatedColors = {
      ...
      [key]: {
        bg: newColor,
        text: getContrastTextColor(newColor),
  ...
  return (
    ...
    >
      <div className="min-h-screen bg-gray-50 p-1 sm:p-1 md:p-2 space-y-1 sm:space-y-2">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 rounded border px-2 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-1 focus:ring-gray-900 dark:focus:ring-gray-100 ${!permissions.canUpdate
                        ...
                        : useCustomTimeSlots
                          ? "border-purple-500 dark:border-purple-400 bg-purple-50 dark:bg-purple-100 text-purple-700 dark:text-purple-500 hover:bg-purple-100 dark:hover:bg-purple-200"
                  ...
                  {permissions.canCreate && (
                    <div className="relative">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded border border-red-500 dark:border-red-400 bg-red-50 dark:bg-red-100 px-2 py-1 text-xs font-medium text-red-700 dark:text-red-800 hover:bg-red-100 dark:hover:bg-red-200 transition-colors focus:outline-none focus:ring-1 focus:ring-red-500"
                        title="Block time slots so no new appointments can be booked"
                      >
                        {blockedSlotsLoading ? (
                        ...
                        {blockedSlots.length > 0 && (
                          <span className="ml-1 text-[10px] bg-red-200 text-red-800 rounded-full px-1.5">
                            {blockedSlots.length}
                          </span>
                        )}
                      </button>
                    ...
                    {orderedColumns.map((column, index) => {
                      ...
                      if (column.type === "doctor") {
                        ...
                        return (
                          <div
                            key={columnKey}
                            className={`sticky top-0 z-[40] bg-white dark:bg-gray-800 border-b ${isLast ? "" : "border-r"} border-gray-200 dark:border-gray-700 p-1.5 relative transition-all ${isDragged ? "opacity-50" : ""
                          ...
                          >
                            {permissions.canRead ? (
                              <div className="flex items-center gap-1">
                                <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-blue-50 dark:bg-blue-900/50 border border-blue-200 dark:border-blue-600 flex items-center justify-center text-blue-700 dark:text-blue-300 font-semibold text-[8px] sm:text-[9px] flex-shrink-0">
                                  {getInitials(doctor.name)}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-[8px] sm:text-[9px] font-semibold text-gray-900 dark:text-gray-100 truncate">
                                    {doctor.name}
                                  </p>
                                </div>
                                {permissions.canUpdate && (
                                  ...
                                  >
                                    <svg
                                      className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-gray-500 dark:text-gray-400"
                                      fill="none"
                                      stroke="currentColor"
                                      viewBox="0 0 24 24"
                                    >
                        ...
                        return (
                          <div
                            key={columnKey}
                            className={`sticky top-0 z-[40] bg-emerald-50 dark:bg-gray-800 border-b ${isLast ? "" : "border-r"} border-gray-200 dark:border-gray-700 p-1.5 transition-all room-column ${isDragged ? "opacity-50" : ""
                              } ${draggedColumnId ? "cursor-move" : ""}`}
                            style={{ height: "36px", gridColumn, gridRow: "1" }}
                            draggable={permissions.canUpdate}
                            onDragStart={(e) => {
                          ...
                          >
                            {permissions.canRead ? (
                              <div className="flex items-center gap-1">
                                <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-emerald-50 dark:bg-emerald-900/50 border border-emerald-200 dark:border-emerald-600 flex items-center justify-center text-emerald-700 dark:text-emerald-300 font-semibold text-[8px] sm:text-[9px] flex-shrink-0">
                                  🏥
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-[8px] sm:text-[9px] font-semibold text-gray-900 dark:text-gray-100 truncate">
                    ...
                    {timeSlots.map((slot, rowIndex) => {
                      ...
                      return (
                        <React.Fragment key={slot.time}>
                          {/* Time label cell */}
                          <div
                            className="sticky left-0 z-[30] bg-white dark:bg-gray-800 border-b border-r border-gray-200 dark:border-gray-700 p-1 relative"
                            style={{
                          ...
                          >
                            <p className="text-[8px] sm:text-[9px] font-semibold text-gray-900 dark:text-gray-100">
                              {slot.displayTime}
                            </p>
                            ...
                            if (column.type === "doctor") {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex flex-col h-full">
                                    {[0, SLOT_INTERVAL_MINUTES].map(
                                      (offset) => {
                                        ...
                                        // Check if this sub-slot is blocked
                                        const blockingEntry = getBlockingForCell(
                                          doctor._id,
                                          null,
                                          subStartMinutes,
                                          subEndMinutes,
                                        );
                                        const isBlocked = !!blockingEntry;
                                          ...
                                          <div
                                            ...
                                            className={`flex-1 transition-all ${isDragOverSlot
                                              ...
                                              : isSelected
                                                ...
                                                : isBlocked
                                                  ? "bg-red-100 dark:bg-red-200 border-l-2 border-red-500 dark:border-red-600 cursor-not-allowed"
                                                  : canBookSlot
                                            ...
                                            title={
                                              isBlocked
                                                ? `Blocked${blockingEntry?.reason ? `: ${blockingEntry.reason}` : ""}${blockingEntry?.blockedByName ? ` (by ${blockingEntry.blockedByName})` : ""}`
                                            ...
                                            onClick={(_e) => {
                                              ...
                                              // If this sub-slot is blocked, offer unblock instead of booking
                                              if (isBlocked && blockingEntry) {
                                                setUnblockConfirm(blockingEntry);
                                                return;
                                              }
                                              if (!canBookSlot) {
                                                const limitTime =
                                  ...
                                  {/* Blocked slot overlay (doctor) */}
                                  {permissions.canRead &&
                                    getRowBlockedSegments(
                                      doctor._id,
                                      null,
                                      rowStartMinutes,
                                      rowStartMinutes + ROW_INTERVAL_MINUTES,
                                      ...
                                      const fromOffset =
                                        seg.fromMinutes - rowStartMinutes;
                                      const toOffset =
                                        seg.toMinutes - rowStartMinutes;
                                      const topPx =
                                        (fromOffset / ROW_INTERVAL_MINUTES) *
                                        ROW_HEIGHT_PX;
                                      ...
                                      return (
                                        <div
                                          key={`block-${seg.block._id}-${slot.time}-${doctor._id}`}
                                          className="absolute left-0.5 right-0.5 border border-red-500/70 bg-red-200/40 dark:bg-red-300/30 rounded-sm flex items-center px-1.5 py-0.5 pointer-events-auto cursor-pointer hover:bg-red-200/60 dark:hover:bg-red-300/50 transition-colors"
                                          style={{
                                            top: `${topPx + 1}px`,
                                            height: `${heightPx}px`,
                                            zIndex: 11,
                                          ...
                                          title={
                                            seg.block.reason
                                              ? `Blocked: ${seg.block.reason}` +
                                              (seg.block.blockedByName
                                                ? ` (by ${seg.block.blockedByName})`
                                                : "")
                                              : seg.block.blockedByName
                                                ? `Blocked by ${seg.block.blockedByName}`
                                                : "Blocked"
                                          }
                                          onClick={(e) => {
                                            e.stopPropagation();
                                                ...
                                                <div className="flex flex-col justify-center h-full">
                                                  {height < 60 ? (
                                                    <div className="flex items-center h-full px-1">
                                                      <span className="text-[11px] font-[600] leading-[1.2] tracking-tight truncate" style={{ color: statusColor.text }}>
                                                        {apt.patientName}
                                                      </span>
                                                    </div>
                                                  ) : (
                                                    <div className="px-1 py-0.5 leading-[1.2] space-y-[1px]">
                                                      ...
                                                      <span className="text-[11px] font-[600] block truncate" style={{ color: statusColor.text }}>
                                                        {layout.totalColumns > 1
                                                          ? apt.patientName.split(" ").slice(0, 2).join(" ")
                                                          : apt.patientName}
                                                      </span>
                                                      {apt.serviceNames && apt.serviceNames.length > 0 ? (
                                                        <span className="text-[10px] font-[400] opacity-[0.8] block truncate" style={{ color: statusColor.text }}>
                                ...
                                >
                                  ...
                                  {permissions.canRead &&
                                    getRowBlockedSegments(
                                      ...
                                      return (
                                        <div
                                          key={`block-${seg.block._id}-${slot.time}-${room._id}`}
                                          className="absolute left-0.5 right-0.5 border border-red-500/70 bg-red-200/40 dark:bg-red-300/30 rounded-sm flex items-center px-1.5 py-0.5 pointer-events-auto cursor-pointer hover:bg-red-200/60 dark:hover:bg-red-300/50 transition-colors"
                                          style={{
                                            top: `${topPx + 1}px`,
                                            height: `${heightPx}px`,
        ...
        {permissions.canCreate && (
          <BlockSlotModal
            isOpen={blockSlotModal.isOpen}
            onClose={() => setBlockSlotModal({ ...blockSlotModal, isOpen: false })}
            onSuccess={() => {
              loadBlockedSlots();
            }}
            defaultDate={selectedDate}
            defaultFromTime={blockSlotModal.defaultFromTime}
            defaultToTime={blockSlotModal.defaultToTime}
            defaultDoctorId={blockSlotModal.defaultDoctorId}
            defaultRoomId={blockSlotModal.defaultRoomId}
            doctorStaff={doctorStaff}
            rooms={rooms}
            getAuthHeaders={getAuthHeaders}
          />
        ...
        {unblockConfirm && (
          <ModalPortal>
            <div
              ...
              onClick={() => !unblockLoading && setUnblockConfirm(null)}
            >
              <div
                className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-4 sm:p-5"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Restore booking for this time range.
                    </p>
                  </div>
                  <button
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-2.5 mb-3 text-xs space-y-1">
                  <div className="flex justify-between gap-2">
                    <span className="text-gray-500">Date</span>
                    <span className="font-medium text-gray-900">
                      {(() => {
                        const raw = unblockConfirm.startDate;
                  ...
                  <div className="flex justify-between gap-2">
                    ...
                    <span className="font-medium text-gray-900">
                      {unblockConfirm.fromTime} - {unblockConfirm.toTime}
                    </span>
                  </div>
                  {unblockConfirm.reason && (
                    <div className="flex justify-between gap-2">
                ...
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <button
                    ...
                    disabled={unblockLoading}
                    className="px-3 py-1.5 text-xs font-medium rounded-lg bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                  >
                    {unblockLoading && (
                      <Loader2 className="w-3 h-3 animate-spin" />
                    )}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-md shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              >
                <div className="flex items-center justify-between gap-1">
                  <p className="text-[10px] font-bold truncate" style={{ color: "inherit" }}>
              ...
              <div className="p-2 space-y-1 max-h-[300px] overflow-y-auto">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[10px] text-gray-700 dark:text-gray-200 font-semibold">
                ...
                <div className="space-y-0.5 pt-0.5 border-t border-gray-100 dark:border-gray-700">
                  {hoveredAppointment.appointment.patientEmrNumber && (
                    <div className="flex items-center gap-1">
                      <span className="text-[9px] text-gray-700 dark:text-gray-300 font-medium">
                        EMR:
                      </span>
                  ...
                  return (
                    <div className="space-y-0.5 pt-0.5 border-t border-gray-100 dark:border-gray-700">
                      {hoveredAppointment.appointment.patientMobileNumber && (
                        <div className="flex items-center gap-1">
                          <span className="text-[9px] text-gray-700 dark:text-gray-300 font-medium w-12 flex-shrink-0">
                            Mobile:
                          </span>
                          <span className="text-[10px] text-gray-700 dark:text-gray-200 truncate">
                            {hoveredAppointment.appointment.patientMobileNumber}
                ...
                <div className="space-y-0.5 pt-0.5 border-t border-gray-100 dark:border-gray-700">
                  ...
                  {hoveredAppointment.appointment.serviceNames &&
                    ...
                    <div className="flex items-start gap-1">
                      ...
                      <span className="text-[10px] text-gray-700 dark:text-gray-200 leading-tight">
                ...
                {hoveredAppointment.appointment.notes && (
                  <div className="pt-0.5 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-[9px] font-semibold text-gray-700 dark:text-gray-300 uppercase mb-0.5">
                      Notes
                    </p>
