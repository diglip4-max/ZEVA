...
const InvoiceUpdateSystem = () => {
  ...
  useEffect(() => {
    const fetchClinicCurrency = async () => {
      try {
        const res = await fetch('/api/clinics/myallClinic', {
          headers: { Authorization: `Bearer ${clinicToken}` }
        });
        if (res.ok) {
          const data = await res.json();
          if (data.success && data.clinic?.currency) {
            setCurrency(data.clinic.currency);
          }
        }
      } catch (e) {
  ...
  }, [formData, invoiceInfo, clinicToken, currentUser.name]);
  ...
  }, [claimStatusModal, invoiceInfo, currentUser.name, formData.advanceClaimReleaseDate, formData.advanceClaimReleasedBy, clinicToken]);
  ...
  }, [formData.status, formData.rejectionNote, formData.advanceClaimReleaseDate, formData.advanceClaimReleasedBy, invoiceInfo, clinicToken, currentUser.name]);
        ...
        <div className="bg-white rounded-xl md:rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-5 lg:space-y-6">
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl shadow-sm border border-gray-200 p-4 md:p-5 lg:p-6">
              <h2 className="text-base md:text-lg font-bold text-gray-900 mb-3 md:mb-4 flex items-center gap-2">
                <CircleDollarSign className="w-4 h-4 md:w-5 md:h-5 text-indigo-600" />
                Payment Details
              </h2>
