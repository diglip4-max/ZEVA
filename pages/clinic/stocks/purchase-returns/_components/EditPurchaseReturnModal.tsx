...
import { Pencil, X, ChevronDown, Search, Trash2, Plus } from "lucide-react";
...
const EditPurchaseReturnModal: React.FC<Props> = ({
  isOpen,
  onClose,
  record,
  onSuccess,
  ...
  const [formData, setFormData] = useState({
    branch: "",
    purchaseOrder: "", // Fixed field name
    supplier: "",
    date: "",
    notes: "",
    status: "Returned",
  ...
  useEffect(() => {
    if (record) {
      setFormData({
        ...
        purchaseOrder:
          record.purchasedOrder?._id || record.purchasedOrder || "",
  ...
  const selectedPO = purchaseOrders.find(
    (p) => p._id === formData.purchaseOrder,
  );
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!formData.branch || !formData.purchaseOrder) {
    ...
    try {
      ...
      const res = await axios.put(
        `/api/stocks/purchase-returns/update-purchase-return/${record._id}`,
        {
          ...formData,
          // Ensure field names match backend schema
          items: items,
          purchasedOrder: formData.purchaseOrder, // Send as purchasedOrder if backend expects that
        },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleCurrentItemChange = (field: keyof ReturnItem, value: any) => {
    ...
    if (field === "itemId") {
      const item: any = stockItems.find((i) => i._id === value);
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Purchase Order - Searchable Select */}
        ...
        <div className="bg-gray-50 px-4 py-3 flex justify-end gap-3">
          ...
          <button
            ...
            disabled={loading || !formData.branch || !formData.purchaseOrder}
