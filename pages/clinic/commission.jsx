...
function ClinicCommissionPage() {
        ...
        try {
          ...
          if (res.data.success) {
            if (
              ...
              const modulePermission = findCommissionModule(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setMembershipList(
          Array.isArray(mRes.data?.memberships) ? mRes.data.memberships : [],
        );
        setPackageList(
    ...
    if (row.patientId && !patientInfoMap[row.patientId]) {
      ...
      try {
        const [basicRes, fullRes, pkgUsageRes] = await Promise.all([
          ...
          axios.get(`/api/staff/get-patient-data/${row.patientId}`, {
            headers,
          }),
          axios
            .get(`/api/clinic/package-usage/${row.patientId}`, { headers })
            .catch(() => ({ data: null })),
        ]);
        ...
        await Promise.all(
          Array.from(transferIds).map(async (pid) => {
            try {
              const r = await axios.get(`/api/staff/get-patient-data/${pid}`, {
                headers,
              });
              const d = r.data;
              const name =
        ...
        const entries = [];
        const existing = Array.isArray(full?.memberships)
          ? full.memberships
          : [];
        ...
        transferredIns.forEach((t) => {
          const dup = displayMemberships.some(
            (m) =>
              String(m.membershipId) === String(t.membershipId) &&
              (!!m.startDate
                ? String(m.startDate) === String(t.startDate)
                : true) &&
          ...
          entries.map(async (e) => {
            ...
            try {
              const mRes = await axios.get(
                `/api/clinic/membership-usage/${row.patientId}?${qs.toString()}`,
                { headers },
              );
              const key = `${e.membershipId}|${e.startDate}|${e.endDate}`;
              membershipUsageMap[key] =
                mRes.data && mRes.data.success ? mRes.data : null;
            } catch {
              const key = `${e.membershipId}|${e.startDate}|${e.endDate}`;
        ...
        setPatientInfoMap((prev) => ({
          ...
          [row.patientId]: {
            ...
            membershipUsageMap,
            displayMemberships,
            packageUsage: pkgUsageRes?.data?.success
              ? pkgUsageRes.data.packageUsage
              : null,
            error: null,
          },
        }));
        ...
        setPatientInfoMap((prev) => ({
          ...prev,
          [row.patientId]: { data: null, error: "Failed to load patient" },
        }));
      }
    }
  };
  ...
  const handleAddExpense = async (commissionId) => {
    ...
    if (validExpenses.length === 0) {
      showToast(
        "Enter at least one valid expense with name and price",
        "error",
      );
      return;
    }
  ...
  const handleToggleSubmit = async (commissionId) => {
    ...
    try {
      const res = await axios.post(
        "/api/clinic/commissions/toggle-submit",
        { commissionId },
        { headers },
      );
      if (res.data.success) {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (selectedPerson && selectedPerson.personId === personId) {
          setModalItems((prev) =>
            prev.map((item) =>
              item.isSubmitted ? { ...item, isApproved: true } : item,
            ),
          );
        }
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-6">
      {toast && (
        <div
          className={`fixed top-4 right-4 px-4 py-3 rounded-lg text-white shadow-lg flex items-center gap-2 ${toast.type === "success" ? "bg-green-600" : "bg-red-600"}`}
        >
          {toast.type === "success" ? (
      ...
      <div className="max-w-9xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
          ...
          <div className="bg-gray-50 rounded-lg p-3 sm:p-4 mb-4 border border-gray-200">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="flex-1 flex flex-col gap-1">
                <label className="text-xs font-medium text-gray-600 dark:text-gray-600">
                  Doctor/Staff Name
                </label>
                <input
                  type="text"
          ...
          {loading ? (
            <div className="text-sm text-teal-600 dark:text-teal-600">
              Loading...
            </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-2">
                          ...
                          {permissions.canUpdate && (
                            <button
                              ...
                              onClick={() =>
                                handleApprove(row.personId, row.source)
                              }
                              title={
                                Number(row.pendingApprovalCount || 0) > 0
                ...
                {modalLoading ? (
                  <div className="text-center py-4">
                    <div className="inline-flex items-center px-3 py-1.5 bg-teal-50 rounded-lg">
                      ...
                      <span className="text-teal-700 dark:text-teal-300 font-medium text-[10px]">
                        Loading history...
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-center py-4">
                    <div className="text-teal-600 dark:text-teal-400 text-[10px]">
                      No records found
                    </div>
                  </div>
                ) : (
                              ...
                              <td className="px-1.5 py-1.5 whitespace-nowrap">
                                <div className="flex items-center gap-1">
                                  ...
                                  {permissions.canUpdate && (
                                    <button
                                      onClick={() =>
                                        handleToggleSubmit(it.commissionId)
                                      }
                                      title={
                                        it.isSubmitted
                                          ? "Unmark submission"
                              ...
                              <tr className="bg-gray-50/60">
                                <td colSpan={12} className="px-1.5 py-1.5">
                                  <div className="rounded-lg border border-gray-200 bg-white p-1.5 shadow-sm overflow-hidden">
                                            ...
                                            {Array.isArray(
                                              ...
                                              it.selectedPackageTreatments
                                                ...
                                                <span className="inline-flex items-center px-1 py-0.5 rounded-full text-[8px] bg-cyan-100 text-cyan-700 font-medium">
                                                  Sess used:{" "}
                                                  {it.selectedPackageTreatments.reduce(
                                                    (s, t) =>
                                                      s +
                                                      (Number(t.sessions) || 0),
                                                    0,
                                      ...
                                      it.selectedPackageTreatments.length >
                                        ...
                                        <div className="mb-1.5">
                                          ...
                                          <div className="flex flex-wrap gap-1">
                                            {it.selectedPackageTreatments.map(
                                              (pt, idx) => (
                                                <span
                                                  key={idx}
                                                  className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] bg-gray-100 text-gray-700"
                                                >
                                                  {pt.treatmentName} —{" "}
                                                  {pt.sessions} sess
                                                </span>
                                    ...
                                    {Array.isArray(it.postCommissionExpenses) &&
                                      it.postCommissionExpenses.length > 0 && (
                                        <div className="mt-1">
                                          <div className="text-[9px] text-gray-600 font-semibold mb-0.5">
                                            Post-commission expenses:
                                          </div>
                                          <div className="flex flex-wrap gap-1">
                                            {it.postCommissionExpenses.map(
                                              (pce, iPce) => (
                                                <span
                                                  key={`pce-${iPce}`}
                                                  className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] bg-violet-100 text-violet-800 border border-violet-200"
                                                >
                                                  {pce.name}:{" "}
                                                  {getCurrencySymbol(currency)}
                                                  {Number(
                                    ...
                                    {permissions.canUpdate && (
                                      <div className="mt-2">
                                        {addExpenseRow !== it.commissionId ? (
                                          <button
                                            onClick={() => {
                                              setAddExpenseRow(it.commissionId);
                                              setNewExpenses([
                                                { name: "", price: "" },
                                              ]);
                                            }}
                                    ...
                                    {it.patientId &&
                                      patientInfoMap[it.patientId]?.full &&
                                      (() => {
                                        const full =
                                          patientInfoMap[it.patientId].full;
                                        const outMemIds = new Set(
                                          (full.membershipTransfers || [])
                                            .filter((t) => t.type === "out")
                                            .map((t) => String(t.membershipId)),
                                        );
                                        const outPkgIds = new Set(
                                          (full.packageTransfers || [])
                                            .filter((t) => t.type === "out")
                                            .map((t) => String(t.packageId)),
                                        );
                                        const showMembership =
                                          full.membership === "Yes" &&
                                          full.membershipId &&
                                          !outMemIds.has(
                                            String(full.membershipId),
                                          );
                                        const showPackage =
                                          full.package === "Yes" &&
                                          full.packageId &&
                                          !outPkgIds.has(
                                            String(full.packageId),
                                          );
                                        if (!showMembership && !showPackage)
                                          return null;
                                        return (
                                          <div className="mt-1 flex flex-wrap items-center gap-1">
                                            {showMembership && (
                                        ...
                                        <div className="mt-1.5 grid grid-cols-1 md:grid-cols-2 gap-1.5">
                                            ...
                                            return filtered.map(
                                              (mItem, idx) => {
                                                ...
                                                const values = Object.entries(
                                                  ...
                                                  .map(([, v]) => v)
                                                  .filter(Boolean);
                                                const totalFree = values.reduce(
                                                  (s, u) =>
                                                    s +
                                                    (u.totalFreeConsultations ||
                                                      0),
                                                  0,
                                                );
                                                const usedFree = values.reduce(
                                                  (s, u) =>
                                                    s +
                                                    (u.usedFreeConsultations ||
                                                      0),
                                                  0,
                                                );
                                                ...
                                                const isOpen =
                                                  !!expandedMemberships[memKey];
                                                return (
                                                  <div
                                                    key={idx}
                                                    className="rounded-md border border-purple-200 bg-purple-50/40 p-1.5"
                                                  >
                                                    <div className="text-[9px] font-semibold text-purple-800 mb-0.5">
                                                      Membership
                                                    </div>
                                                    <div className="flex flex-wrap items-center gap-1">
                                                      <span className="inline-flex px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-700 text-[8px]">
                                                        {name}
                                                      </span>
                                                      ...
                                                      {inTransferred && (
                                                        <span className="inline-flex px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[8px]">
                                                          Transferred in
                                                        </span>
                                                      )}
                                                      <button
                                                        onClick={() =>
                                                          setExpandedMemberships(
                                                            (prev) => ({
                                                              ...prev,
                                                              [memKey]:
                                                                !prev[memKey],
                                                    ...
                                                    {isOpen && (
                                                      <div className="mt-2 rounded-md bg-white border border-purple-100 p-2">
                                                        <div className="text-[11px] text-gray-700">
                                                          ...
                                                          {remaining > 0 ? (
                                                            <span className="ml-2 inline-flex px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">
                                                              Remaining{" "}
                                                              {remaining}
                                                            </span>
                                                          ) : null}
                                                        ...
                                                        {m?.benefits && (
                                                          <div className="mt-1 flex flex-wrap gap-1.5">
                                                            {m.benefits
                                                              .priorityBooking ? (
                                                              <span className="inline-flex px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[11px]">
                                                                Priority booking
                                                              </span>
                                                            ) : null}
                                                            {typeof m.benefits
                                          ...
                                          {(() => {
                                                  ...
                                                  return (
                                                    ...
                                                    >
                                                      ...
                                                      <div className="flex flex-wrap items-center gap-2">
                                                        <span className="inline-flex px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[11px]">
                                                          {pkg.packageName}
                                                        </span>
                                                        {inTransferred && (
                                                          <span className="inline-flex px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[11px]">
                                                            ...
                                                            <div className="flex flex-wrap gap-1.5">
                                                              {pkg.treatments.map(
                                                                (t, i2) => (
                                                                  <span
                                                                    key={i2}
                                                                    className="inline-flex px-2 py-0.5 rounded bg-gray-100 text-gray-700 text-[11px]"
                                                                  >
                                                                    {
                                                                      t.treatmentName
                                                                    }{" "}
                                            ...
                                            const assigned = Array.isArray(
                                              ...
                                              ? full.packages
                                              : [];
                                            const mainPkgId = full.packageId
                                              ? String(full.packageId)
                                              : null;
                                            ...
                                            if (mainPkgId)
                                              allAssignedIds.add(mainPkgId);
                                            const filteredIds = Array.from(
                                              allAssignedIds,
                                            ).filter(
                                              (pid) =>
                                                !transfersOutIds.has(pid),
                                            );
                                            if (filteredIds.length === 0)
                                              return null;
                                            return filteredIds.map(
                                              (pid, idx) => {
                                                const pkgDef = packageList.find(
                                                  (x) =>
                                                    String(x._id) ===
                                                    String(pid),
                                                );
