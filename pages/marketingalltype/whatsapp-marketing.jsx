      ...
      <div className="max-w-7xl mx-auto space-y-3">
        ...
        <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-gray-600">WhatsApp Marketing</p>
              <h1 className="text-lg sm:text-xl font-bold text-gray-900 mt-0.5">WhatsApp Campaign Broadcast</h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-0.5">Engage your customers with targeted WhatsApp campaigns</p>
            </div>
            <div className="flex items-center gap-2 text-xs text-gray-600">
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-green-50 border border-green-200 px-2.5 py-1.5">
                ...
                <span className="font-medium text-green-700">WhatsApp Ready</span>
              </span>
            </div>
          </div>
          <div className="h-0.5 bg-gradient-to-r from-green-500 via-green-600 to-green-500 rounded-full mt-3"></div>
        </section>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gray-50 border-b border-gray-200 px-3 sm:px-4 py-3">
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">Campaign Type</label>
          ...
          <div className="p-3 sm:p-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              ...
              <div className="space-y-4">
                {mode === "single" ? (
                  <div>
                    <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Recipient Number</label>
                    <input
                      type="text"
                      placeholder="+919876543210"
                      value={singleNumber}
                      onChange={(e) => setSingleNumber(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500/20 focus:border-green-600 transition-all text-xs sm:text-sm text-gray-900 placeholder:text-gray-400"
                    />
                  </div>
                ) : (
                  <div>
                    <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">
                      Recipient Numbers <span className="text-gray-500 font-normal">(comma separated)</span>
                    </label>
                    <textarea
                      placeholder="+919876543210, +919812345678"
                      value={bulkNumbers}
                      onChange={(e) => setBulkNumbers(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500/20 focus:border-green-600 transition-all resize-none text-xs sm:text-sm text-gray-900 placeholder:text-gray-400"
                      rows={4}
                    />
                    {bulkNumbers && (
                      <p className="text-xs text-gray-600 mt-1">
                        {bulkNumbers.split(",").filter(n => /^\+?\d{10,15}$/.test(n.trim())).length} valid number(s) detected
                      </p>
                    )}
                  </div>
                )}
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Campaign Title</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Message Content</label>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Campaign Image</label>
                  <div className="border-2 border-dashed border-gray-200 rounded-lg p-4 sm:p-6 hover:border-green-400 transition-colors bg-gray-50">
                    <input 
                      type="file" 
                      accept="image/*" 
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-3 sm:p-4">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs sm:text-sm font-semibold text-gray-900 mb-1">Pro Tip</h4>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              <button
                ...
                className="w-full bg-green-600 text-white font-medium py-3 rounded-lg shadow-sm hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-xs sm:text-sm"
              >
                {loading ? (
