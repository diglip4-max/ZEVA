...
const DoctorOffersPage = () => {
  ...
  const fetchOffers = useCallback(async () => {
    ...
    try {
      const cached =
        typeof window !== "undefined"
          ? sessionStorage.getItem("doctorOffersCache")
          : null;
      if (cached) {
      ...
      console.warn("Failed to read offers cache:", error);
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        try {
          ...
          console.warn("Failed to write offers cache:", error);
        }
  ...
  }, [permissions.canRead, permissionsLoaded, token]);
  ...
  const handleOfferSaved = (offer, isUpdate) => {
    setOffers((prev) => {
      ...
      try {
        ...
        console.warn("Failed to update offers cache:", error);
      }
      return updated;
    });
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (
      typeof window !== "undefined" &&
      !window.confirm("Are you sure you want to delete this offer?")
    ) {
      return;
    }
    if (!token) {
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2.5">
            <div>
              ...
              <p className="text-gray-600 text-xs">
                Create and manage promotional offers for your patients
              </p>
            </div>
...
DoctorOffersPage.getLayout = (page) => (
  <DoctorLayout>{page}</DoctorLayout>
);

const ProtectedDoctorOffersPage = withDoctorAuth(DoctorOffersPage);
ProtectedDoctorOffersPage.getLayout = DoctorOffersPage.getLayout;

export default ProtectedDoctorOffersPage;
