  ...
  try {
    ...
    if (me.role === "clinic") {
      ...
      if (!clinicId)
        return res
          .status(400)
          .json({ success: false, message: "clinicId is required for admin in query parameters" });
    }
    ...
    res.status(500).json({ success: false, message: "Failed to delete purchase invoice", error: error.message });
  }
}
