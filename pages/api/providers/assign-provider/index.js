...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!requireRole(me, ["clinic"])) {
      return res.status(403).json({ success: false, message: "Access denied. Only clinic users can assign providers." });
    }
    ...
    if (users.length !== userIds.length) {
      return res.status(400).json({ success: false, message: "One or more users are not valid agents of this clinic" });
    }
    ...
    const updatedProvider = await Provider.findByIdAndUpdate(
      ...
      { $set: { owners: userIds } },
      { new: true }
    ).select("-secrets -_ac -_ct").populate("owners", "_id name email role");
