...
export default async function handler(req, res) {
  ...
  try {
    ...
    // find user with allowed roles (clinic, doctor staff, agent)
    const user = await User.findOne({
      email,
      role: { $in: ["clinic", "doctorStaff", "agent"] },
    });
    ...
    return res.status(200).json({
      ...
      token,
      tokenKey: user.role === "agent" ? "agentToken" : user.role === "clinic" ? "clinicToken" : "userToken",
