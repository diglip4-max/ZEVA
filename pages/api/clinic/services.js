...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const services = await Service.find({ clinicId }).sort({ createdAt: -1 }).lean();
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!serviceSlug || !serviceSlug.trim()) {
        return res.status(400).json({ success: false, message: "Service slug is required" });
      }
      ...
      const exists = await Service.findOne({ clinicId, name: name.trim() });
      if (exists) {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const { serviceId, name, serviceSlug, price, durationMinutes, isActive } = req.body;
      if (!serviceId) {
      ...
      if (duplicate) {
        return res.status(400).json({ success: false, message: "Another service with this name exists" });
      }

      service.name = name.trim();
      service.serviceSlug = serviceSlug.trim();
