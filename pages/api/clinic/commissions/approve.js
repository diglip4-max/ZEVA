...
/**
 * POST /api/clinic/commissions/approve
 * Body: { staffId }
 * Approves all isSubmitted=true commissions for the given staffId in this clinic.
 * Sets isApproved=true on every submitted commission for that staff member.
 * After approval, those commissions become visible in the staff member's AgentHeader wallet.
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // Approve all isSubmitted=true commissions for this staff member in this clinic
    const result = await Commission.updateMany(
      { clinicId, staffId, isSubmitted: true, isApproved: false },
      { $set: { isApproved: true } }
    );
