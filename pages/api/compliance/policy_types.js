...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const items = await PolicyType.find({ clinicId }).sort({ name: 1 }).lean();
  ...
  if (req.method === "POST") {
    ...
    try {
      const item = await PolicyType.create({ clinicId, name: String(name).trim(), status });
      return res.status(201).json({ success: true, item });
    } catch (err) {
      if (err && err.code === 11000) {
        return res.status(409).json({ success: false, message: "Type already exists" });
      }
      return res.status(500).json({ success: false, message: err.message || "Failed to create type" });
    }
  }
  ...
  if (req.method === "DELETE") {
    ...
    await PolicyType.deleteOne({ _id: id, clinicId });
