// pages/api/finance/reports/cheques.js
...
export default async function handler(req, res) {
  ...
  try {
    const { bank, dateFrom, dateTo } = req.query;

    const match = { clinicId };
    if (bank) match.bank = bank;
    if (dateFrom || dateTo) {
      match.chequeDate = {};
      if (dateFrom) match.chequeDate.$gte = new Date(dateFrom);
      if (dateTo) match.chequeDate.$lte = new Date(dateTo);
    }
    ...
    const grandTotal = byStatus.reduce((s, c) => s + c.totalAmount, 0);
    const grandCount = byStatus.reduce((s, c) => s + c.count, 0);
