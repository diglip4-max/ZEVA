...
export async function checkAgentPermission(
  ...
  try {
    ...
    if (
      ...
      agentPermission.permissions.length === 0
    ) {
      return {
        hasPermission: false,
        error: "No permissions found for this agent",
      };
    }
    ...
    modulePermission = agentPermission.permissions.find((p) => {
      const permModule = p.module || "";
      const found = moduleCandidates.some(
        (candidate) => permModule === candidate,
      );
      ...
      for (const permission of agentPermission.permissions) {
        if (
          ...
          const subModule = permission.subModules.find((sm) => {
            ...
            const found = moduleCandidates.some(
              (candidate) => sm.moduleKey === candidate,
            );
            if (found) {
            }
            return found;
    ...
    if (!modulePermission) {
      ...
      modulePermission = agentPermission.permissions.find((p) => {
        ...
        const found = moduleCandidates.some((candidate) => {
          ...
          const permPrefix =
            permModule.match(/^(admin|clinic|doctor)_/)?.[0] || "";
          const candidatePrefix =
            candidate.match(/^(admin|clinic|doctor)_/)?.[0] || "";
      ...
      for (const permission of agentPermission.permissions) {
        if (
          ...
          const subModule = permission.subModules.find((sm) => {
            ...
            const found = moduleCandidates.some(
              (candidate) =>
                sm.moduleKey.replace(/^(admin|clinic|doctor)_/, "") ===
                candidate.replace(/^(admin|clinic|doctor)_/, ""),
            );
            if (found) {
            }
            return found;
            ...
            (sm) =>
              ...
              moduleCandidates.some((candidate) => {
                const candidateName = candidate
                  .replace(/^(admin|clinic|doctor)_/, "")
                  .replace(/_/g, " ")
                  .toLowerCase();
                const subModuleName = sm.name.toLowerCase();
                const found =
                  subModuleName.includes(candidateName) ||
                  candidateName.includes(subModuleName);
                if (found) return found;
              }),
          );
          if (subModule) {
    ...
    if (subModuleName) {
      ...
      if (!subModule) {
        return {
          hasPermission: false,
          error: `Submodule ${subModuleName} not found in permissions`,
        };
      }
...
export async function getAgentModulePermissions(agentId, moduleKey) {
  ...
  try {
    ...
    if (!modulePermission) {
      return {
        permissions: null,
        error: `No permissions found for module: ${moduleKey}`,
      };
    }
