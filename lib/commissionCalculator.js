 ...
 * Handles different commission calculation types including flat, target-based, and after_deduction
 */
...
export async function calculateAfterDeductionCommission({
  ...
  try {
    ...
    // This helps us determine which complaints are NEW since the last billing
    const lastBilling = await Billing.findOne(lastBillingQuery)
      .sort({ createdAt: -1 })
      .select("createdAt invoiceNumber")
      .lean();

    // Determine the cutoff date: only consider complaints after the last billing
    // If no previous billing exists, consider all complaints (first-time billing)
    const cutoffDate = lastBilling?.createdAt || null;

    // Build query to fetch only NEW complaints (created after last billing)
    const complaintsQuery = {
      patientId,
    };

    // Only include complaints created AFTER the last billing date
    if (cutoffDate) {
      complaintsQuery.createdAt = { $gt: cutoffDate };
    }

    // Fetch only NEW complaints (since last billing) for this patient
...
export async function calculateCommissionForStaff({
  ...
  try {
    ...
    if (commissionType === "target_based") {
      ...
      // Other commission types (target_plus_expense) can be implemented here
      return {
        shouldCreateCommission: false,
