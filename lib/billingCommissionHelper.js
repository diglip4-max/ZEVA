...
export async function processBillingCommissions(params) {
  ...
  //
  // A "mixed billing" is when the same invoice contains BOTH:
  //   (A) treatments inherited from the appointment
  //       (carry originalAppointmentQuantity or match appointment.serviceIds)
  //   (B) treatments added directly by the user in the billing UI
  //       (no originalAppointmentQuantity AND not in appointment.serviceIds)
  //
  // At the billing-record level the `directBilling` flag is set to false
  // whenever ANY portion of the billing came from an appointment, so for
  // mixed billings we CANNOT gate commissions on `directBilling` alone.
  // We MUST use the per-treatment amounts computed here so that the
  // appointment portion pays the doctor, and the direct-add portion pays
  // the invoicedBy person.
  // ============================================================
  ...
  if (selectedTreatments && selectedTreatments.length > 0) {
    selectedTreatments.forEach(t => {
      ...
      const matchedByServiceId = tServiceId && appointmentServiceIds.has(tServiceId);
      const hasOriginalQty = typeof t.originalAppointmentQuantity === "number" && t.originalAppointmentQuantity > 0;
      const isFromAppointment = matchedByServiceId || hasOriginalQty;
      const originalQty = hasOriginalQty ? Number(t.originalAppointmentQuantity) : 0;
      const currentQty = Number(t.quantity || 1);
      const unitPrice = Number(t.price || 0);

      if (isFromAppointment) {
        // Up to originalQty counts as the appointment leg (paid to doctor).
        // Any qty above originalQty counts as direct (paid to invoicedBy person).
        const appointmentQty = hasOriginalQty
          ? Math.min(Math.max(currentQty, 0), originalQty)
          : Math.max(currentQty, 0);
        const directQty = Math.max(0, currentQty - appointmentQty);
        appointmentTreatmentsAmount += unitPrice * appointmentQty;
        directTreatmentsAmount += unitPrice * directQty;
      } else {
        // Completely absent from the original appointment → 100% direct.
        directTreatmentsAmount += unitPrice * Math.max(currentQty, 0);
      }
    });
  } else {
    // No line-level detail: fall back to the billing-level flag + serviceAmount.
    // This preserves the pre-existing behavior for legacy billings that do
    // not carry selectedTreatments (e.g. standalone package billing).
    if (directBilling) {
      directTreatmentsAmount = serviceAmount;
    } else {
  ...
  // Derived booleans that are safe for MIXED billing scenarios:
  // `hasAnyAppointmentPortion` = at least $1 came from the appointment (doctor-commissionable)
  // `hasAnyDirectPortion`      = at least $1 was added directly (invoicedBy-commissionable)
  // These replace boolean gating on `directBilling`, which only describes the
  // whole-record intent and can be wrong once the user adds line items.
  const hasAnyAppointmentPortion = appointmentCommissionablePaidAmount > 0;
  const hasAnyDirectPortion = directCommissionablePaidAmount > 0;
  ...
  if (processNewBillingCommission) {
    try {
      ...
      console.log("[Phase 5 - Doctor] directBilling:", directBilling, "hasAnyAppointmentPortion:", hasAnyAppointmentPortion, "doctorId:", appointment?.doctorId);
      console.log("[Phase 5 - Doctor] Conditions: hasAnyAppointmentPortion=", hasAnyAppointmentPortion, "amount>0=", doctorCommissionablePaidAmount > 0, "hasDoctorId=", !!appointment?.doctorId);

      // Gate on the *actual* appointment-line dollars present (hasAnyAppointmentPortion),
      // NOT the billing-level `!directBilling` flag. This is what makes mixed billings
      // work: both halves (appointment + direct) can fire in a single invoice, while
      // pure-direct billings (no appointment lines) skip this block entirely because
      // hasAnyAppointmentPortion will be false.
      if (hasAnyAppointmentPortion && doctorCommissionablePaidAmount > 0 && appointment?.doctorId) {
    ...
    try {
      ...
      console.log("[Phase 6 - BilledPerson] directBilling:", directBilling, "hasAnyDirectPortion:", hasAnyDirectPortion, "billedPersonCommissionablePaidAmount:", billedPersonCommissionablePaidAmount);
      console.log("[Phase 6 - BilledPerson] Conditions: hasAnyDirectPortion=", hasAnyDirectPortion, "amount>0=", billedPersonCommissionablePaidAmount > 0);

      // Gate on the *actual* direct-add dollars present (hasAnyDirectPortion),
      // NOT the billing-level `directBilling` flag. This is the core fix for
      // mixed billings where the invoice-level flag is `directBilling: false`
      // (because an appointment was billed) but the user also added direct
      // treatments on top. Without this change, the direct portion's
      // commission to the invoicedBy person was silently skipped.
      if (hasAnyDirectPortion && billedPersonCommissionablePaidAmount > 0) {
