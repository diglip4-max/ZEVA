...
export async function recomputeBillingCache(billingId, session = null) {
  ...
  // Backward compatibility: also mirror the remaining outstanding amount
  // onto legacy Billing.pending so existing UI (patient-balance, profile-view,
  // AppointmentBillingModal) keeps showing the correct outstanding figure
  // when a clearance happens via the ledger only. The PatientPendingLedger
  // collection is the source of truth.
  //
  // IMPORTANT: We deliberately DO NOT mirror onto Billing.pendingUsed.
  // That field carries different semantics in the existing schema:
  //   Billing.pendingUsed = "amount of OTHER previous pending cleared
  //                          by this invoice at its own creation time"
  // It is NOT "amount paid against this invoice's pending" (which is what
  // ledger.totalPaid represents). Overwriting it would corrupt history.
  //
  // We only touch the legacy `pending` field if THIS billing has at least
  // one ledger row, so we never clobber audit / advance-only billings.
  const update = {
    pendingLedgerCached: totalRemaining,
    pendingLedgerOpenCount: openCount,
  };
  if (allRows.length > 0) {
    update.pending = totalRemaining;
  }
...
/* ============================================================
   ...
   billing.pending - billing.pendingUsed. Returns the created entry.
============================================================ */
export async function syncBillingFromLegacy(billing, { session = null, createdBy = null } = {}) {
  ...
  if (existing) return null; // already has a ledger row

  const remaining = Math.max(0, Number(billing.pending || 0) - Number(billing.pendingUsed || 0));
  if (remaining <= 0) {
